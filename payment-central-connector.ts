...
export class PaymentCentralConnector implements DataSourceConnector {
  ...
  async fetch(config: ConnectorConfig): Promise<FetchResult> {
    ...
    // ── Auto-discover file from S3 prefix ────────────────────────────────────
    const listRes = await s3.send(new ListObjectsV2Command({ Bucket: bucketName, Prefix: prefix, MaxKeys: 10 }));
    const objects = (listRes.Contents || []).filter(o => o.Key && o.Key !== prefix);
    if (objects.length === 0) {
      throw new Error(
        `No Payment Central config file found in S3 under s3://${bucketName}/${prefix} — ` +
        `upload a file using the "Upload File" button before running the pipeline`,
      );
    }
    // Use the most recently modified file if multiple exist
    objects.sort((a, b) => (b.LastModified?.getTime() ?? 0) - (a.LastModified?.getTime() ?? 0));
    const s3Key   = objects[0].Key!;
    const fileName = s3Key.slice(prefix.length);
    const fmt      = 'auto';

    // ── Fetch file from S3 ────────────────────────────────────────────────────
    console.log(`[payment-central-connector] fetching s3://${bucketName}/${s3Key} (auto-discovered)`);
