        ...
        try {
          ...
          const relLookupQuery = `
            ...
            SELECT ?relName ?relType ?srcEntityId ?srcEntity ?tgtEntityId ?tgtEntity WHERE {
              ?rel env:id "${body.relationshipId}" .
          ...
          try {
            ...
            if (b) {
              relDetails = {
                name: b.relName?.value || body.relationshipId,
          ...
          if (isIntegrationRel && cascadeIntegrationId) {
            try {
              ...
              for (const cb of (cascadeResult?.results?.bindings || [])) {
                ...
                if (siblingRelId && siblingRelId !== body.relationshipId) {
                  try {
          ...
          if (isIntegrationRel) {
            await neptuneClient.logHistory(
              envId || body.relationshipId,
              envName || integrationName || body.relationshipId,
              'delete-integration',
              {
                integrationName,
                targetEnvName,
                targetEnvId: relDetails.targetEntityId || '',
                removedBy: body.user || 'system',
              },
              body.user || 'system'
            );
          }
