...
async function storeOptionalEntityProps(entityId: string, body: any): Promise<void> {
  const optionalProps = ['primaryUsage', 'currentUsage', 'collaborators', 'currentBuild', 'scheduledUpdates', 'environmentType'];
  ...
  // Store custom metadata as meta_<key> triples
  if (body.customMetadata && typeof body.customMetadata === 'object') {
    const entries = Object.entries(body.customMetadata as Record<string, string>).slice(0, 10);
    for (const [key, value] of entries) {
      const safeKey = key.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 50);
      if (safeKey && value !== undefined && value !== null) {
        triples.push(`env:${entityId} env:meta_${safeKey} "${escapeSparql(String(value))}" .`);
      }
    }
  }
  if (triples.length > 0) {
      ...
      return {
        ...
        body: JSON.stringify({ 
          ...
          availableOperations: [
            ...
            'update-entity', 'bulk-ingest', 'patch-scheduled-updates'
          ]
        }),
      };
    }
      ...
      case 'update-entity':
        ...
        try {
          ...
          const getOldPropsQuery = `
            ...
            SELECT ?oldName ?oldDescription ?oldOwner ?oldStatus ?oldPrimaryUsage ?oldCurrentUsage ?oldCollaborators ?oldCurrentBuild ?oldScheduledUpdates ?oldEnvironmentType WHERE {
              ...
              OPTIONAL { <${entityUri}> env:environmentType ?oldEnvironmentType }
            }
          `;
          ...
          const updateQuery = `
            ...
            DELETE {
              ...
              <${entityUri}> env:environmentType ?oldEnvironmentType .
            }
            INSERT {
              ...
              ${body.environmentType ? `<${entityUri}> env:environmentType "${escapeSparql(body.environmentType)}" .` : ''}
            }
            WHERE {
              OPTIONAL { <${entityUri}> env:name ?oldName }
          ...
          // Delete all existing meta_* properties and replace with new ones
          const deleteMetaQuery = `
            PREFIX env: <http://neptune.aws.com/envmgmt/ontology/>
            DELETE { <${entityUri}> ?p ?v }
            WHERE  { <${entityUri}> ?p ?v . FILTER(STRSTARTS(STR(?p), "http://neptune.aws.com/envmgmt/ontology/meta_")) }
          `;
          await neptuneClient.executeSparqlUpdate(deleteMetaQuery);
          if (body.customMetadata && typeof body.customMetadata === 'object') {
            const metaEntries = Object.entries(body.customMetadata as Record<string, string>).slice(0, 10);
            const metaTriples: string[] = [];
            for (const [key, value] of metaEntries) {
              const safeKey = key.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 50);
              if (safeKey && value !== undefined && value !== null && String(value).trim() !== '') {
                metaTriples.push(`<${entityUri}> env:meta_${safeKey} "${escapeSparql(String(value))}" .`);
              }
            }
            if (metaTriples.length > 0) {
              const insertMetaQuery = `PREFIX env: <http://neptune.aws.com/envmgmt/ontology/>\nINSERT DATA { ${metaTriples.join(' ')} }`;
              await neptuneClient.executeSparqlUpdate(insertMetaQuery);
            }
          }
          // ========== END CUSTOM METADATA UPDATE ==========
          ...
          // Read existing config values BEFORE any delete/replace, so the history diff is accurate
          let oldCfgMap: Record<string, string> = {};
          if (body.configurations && typeof body.configurations === 'object' && Object.keys(body.configurations).length > 0) {
            try {
              const oldCfgQuery = `
                PREFIX env: <http://neptune.aws.com/envmgmt/ontology/>
          ...
          // Only touch config properties if the request explicitly includes configurations.
          // If omitted (e.g. a metadata-only update), leave existing configs untouched.
          if (body.configurations && typeof body.configurations === 'object' && Object.keys(body.configurations).length > 0) {
            ...
            const deleteConfigQuery = `
              ...
              WHERE {
                ...
                FILTER(!CONTAINS(STR(?configProp), "config_bp_"))
                FILTER(!CONTAINS(STR(?configProp), "config_bpBy_"))
                FILTER(!CONTAINS(STR(?configProp), "config_bpOn_"))
                FILTER(!CONTAINS(STR(?configProp), "config_orphan"))
                FILTER(!CONTAINS(STR(?configProp), "config_previouslyConnected"))
                FILTER(!CONTAINS(STR(?configProp), "config_tag_"))
              }
            `;
            console.log('Deleting old config properties (pipeline update)');
          ...
          if (body.configurations && typeof body.configurations === 'object' && Object.keys(body.configurations).length > 0) {
            // Build diff from oldCfgMap (read before the delete/replace above)
          ...
          const trackedFieldsMap: Record<string, string> = {
            ...
            environmentType:  'oldEnvironmentType',
          };
          ...
          if (body.businessPurposes && typeof body.businessPurposes === 'object') {
            ...
            const ontoPfx = 'http://neptune.aws.com/envmgmt/ontology/';
            ...
            for (const [cfgKey, bpData] of Object.entries(body.businessPurposes as Record<string, any>)) {
              ...
              deleteTriples    += `  <${entityUri}> <${ontoPfx}config_bp_${safeKey}> ?${vVal} .\n`;
              deleteTriples    += `  <${entityUri}> <${ontoPfx}config_bpBy_${safeKey}> ?${vBy} .\n`;
              deleteTriples    += `  <${entityUri}> <${ontoPfx}config_bpOn_${safeKey}> ?${vOn} .\n`;
              optionalPatterns += `  OPTIONAL { <${entityUri}> <${ontoPfx}config_bp_${safeKey}> ?${vVal} }\n`;
              optionalPatterns += `  OPTIONAL { <${entityUri}> <${ontoPfx}config_bpBy_${safeKey}> ?${vBy} }\n`;
              optionalPatterns += `  OPTIONAL { <${entityUri}> <${ontoPfx}config_bpOn_${safeKey}> ?${vOn} }\n`;

              if (bpValue !== '') {
                insertTriples += `  <${entityUri}> <${ontoPfx}config_bp_${safeKey}> "${escSparql(bpValue)}" .\n`;
                insertTriples += `  <${entityUri}> <${ontoPfx}config_bpBy_${safeKey}> "${escSparql(bpByVal)}" .\n`;
                insertTriples += `  <${entityUri}> <${ontoPfx}config_bpOn_${safeKey}> "${escSparql(bpNow)}" .\n`;
              }
            }

            if (deleteTriples) {
              const batchQuery = `DELETE {
${deleteTriples}} INSERT {
${insertTriples}} WHERE {
  <${entityUri}> <http://neptune.aws.com/envmgmt/ontology/id> ?_anchor .
${optionalPatterns}}`;
      ...
      case 'patch-scheduled-updates': {
        // Patch ONLY the scheduledUpdates field — all other entity properties are preserved.
        // Change/incident numbers are unique keys; existing entries not in the new batch are kept.
        if (!body.id || body.scheduledUpdates === undefined) {
          return {
            statusCode: 400,
            headers,
            body: JSON.stringify({ error: 'Missing required fields: id, scheduledUpdates' }),
          };
        }

        const patchEntityId = body.id.startsWith('http://neptune.aws.com/envmgmt/ontology/')
          ? body.id.replace('http://neptune.aws.com/envmgmt/ontology/', '')
          : body.id;

        const escapeSparqlPatch = (str: string) => {
          if (!str) return '';
          return str.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g, '\\n').replace(/\r/g, '\\r').replace(/\t/g, '\\t');
        };

        /**
         * Parse the free-form scheduledUpdates text into a Map<number, lines[]>.
         * A "block" starts with a line matching /^\[(\S+)\]/ and ends before the next
         * such line, a section header (=== ... ===), or end of string.
         * Section headers are preserved as-is when rebuilding.
         */
        const parseScheduleBlocks = (text: string): Map<string, string[]> => {
          ...
          for (const line of lines) {
            ...
            if (blockStart) {
              if (currentKey) map.set(currentKey, currentLines);
        ...
        /**
         * Extract section headers and their associated entry order from text,
         * then merge two maps (existing wins for non-new keys; incoming wins for its keys).
         * Returns the reconstructed merged text preserving section headers.
         */
        const mergeScheduleTexts = (existing: string, incoming: string): string => {
          const existingBlocks = parseScheduleBlocks(existing);
          const incomingBlocks = parseScheduleBlocks(incoming);

          // Merged: incoming keys replace existing; existing-only keys are kept
          const merged = new Map<string, string[]>(existingBlocks);
          for (const [key, lines] of incomingBlocks) {
            merged.set(key, lines);
          }

          // Determine section headers from incoming text (it has the canonical sections)
          // Fall back to a generic header if no sections in existing
          const incomingLines = incoming.split('\n');
          const existingLines = existing.split('\n');

          // Collect section header → [keys in that section] from incoming
          type Section = { header: string; keys: string[] };
          const sections: Section[] = [];
          let currentSection: Section | null = null;
          for (const line of incomingLines) {
            if (line.trim().startsWith('===')) {
              if (currentSection) sections.push(currentSection);
              currentSection = { header: line, keys: [] };
            } else {
              const m = line.match(/^\[(\S+)\]/);
              if (m && currentSection) currentSection.keys.push(m[1]);
            }
          }
          if (currentSection) sections.push(currentSection);

          // Also collect section→keys from existing for entries not in incoming
          const existingSections: Section[] = [];
          let exSection: Section | null = null;
          for (const line of existingLines) {
            if (line.trim().startsWith('===')) {
              if (exSection) existingSections.push(exSection);
              exSection = { header: line, keys: [] };
            } else {
              const m = line.match(/^\[(\S+)\]/);
              if (m && exSection) exSection.keys.push(m[1]);
            }
          }
          if (exSection) existingSections.push(exSection);

          // Build the output: for each section, add incoming keys first (in order),
          // then existing-only keys for that section header
          const incomingKeySet = new Set(incomingBlocks.keys());
          const outputLines: string[] = [];
          const emittedKeys = new Set<string>();

          for (const sec of sections) {
            outputLines.push(sec.header);
            // Incoming keys for this section
            for (const key of sec.keys) {
              const block = merged.get(key);
              if (block) { outputLines.push(...block, ''); emittedKeys.add(key); }
            }
            // Existing-only keys that belong to the same section header
            const matchingExSec = existingSections.find(e => e.header === sec.header);
            if (matchingExSec) {
              for (const key of matchingExSec.keys) {
                if (!incomingKeySet.has(key) && !emittedKeys.has(key)) {
                  const block = merged.get(key);
                  if (block) { outputLines.push(...block, ''); emittedKeys.add(key); }
                }
              }
          ...
          // Emit any remaining existing-only keys that didn't match any section
          for (const [key, block] of merged) {
            if (!emittedKeys.has(key)) { outputLines.push(...block, ''); }
          }
        ...
        const patchFindQ = `
          PREFIX env: <http://neptune.aws.com/envmgmt/ontology/>
          SELECT ?entity WHERE { ?entity env:id "${escapeSparqlPatch(patchEntityId)}" . }
        `;
        const patchFind = await neptuneClient.executeSparqlQuery(patchFindQ);
        if (!patchFind.results?.bindings?.length) {
          return {
            statusCode: 404,
            headers,
            body: JSON.stringify({ error: `Entity not found: ${patchEntityId}` }),
          };
        }
        const patchUri = patchFind.results.bindings[0].entity.value;

        // Fetch existing scheduledUpdates value
        const existingQ = `
          PREFIX env: <http://neptune.aws.com/envmgmt/ontology/>
          SELECT ?val WHERE { OPTIONAL { <${patchUri}> env:scheduledUpdates ?val } }
        `;
        const existingResult = await neptuneClient.executeSparqlQuery(existingQ);
        const existingText: string = existingResult.results?.bindings?.[0]?.val?.value || '';

        // Merge: keep existing entries not in the new batch, update/add new ones
        const mergedText = mergeScheduleTexts(existingText, body.scheduledUpdates);

        const patchQ = `
          PREFIX env: <http://neptune.aws.com/envmgmt/ontology/>
          DELETE { <${patchUri}> env:scheduledUpdates ?old }
          INSERT { <${patchUri}> env:scheduledUpdates "${escapeSparqlPatch(mergedText)}" }
          WHERE  { OPTIONAL { <${patchUri}> env:scheduledUpdates ?old } }
        `;
        await neptuneClient.executeSparqlUpdate(patchQ);
        result = { patched: patchUri, field: 'scheduledUpdates', merged: mergedText.split('\n').filter(l => /^\[/.test(l)).length };
        break;
      }
      ...
      default:
        return {
          ...
          'create', 'update-entity', 'bulk-ingest', 'patch-scheduled-updates'
        ]
          }),
        };
    }
