...
${optionalPatterns}}`;
      ...
      case 'patch-scheduled-updates': {
        ...
        // scheduledUpdates is a JSON array: [{date,startTime,endDate,endTime,description},...].
        // Change/incident number (e.g. [CHG001]) embedded in description is the unique merge key.
        // Existing entries not present in the incoming batch are kept unchanged.
        ...
        type ScheduleEntry = { date: string; startTime: string; endDate: string; endTime: string; description: string; [k: string]: string };

        /** Extract the unique ticket/change number from a description string, e.g. "[CHG0012345]" → "CHG0012345". */
        const extractKey = (desc: string): string | null => {
          const m = (desc || '').match(/^\[([^\]]+)\]/);
          return m ? m[1] : null;
        };

        /** Parse scheduledUpdates value — accepts JSON array string or already-array. */
        const parseEntries = (raw: string | ScheduleEntry[]): ScheduleEntry[] => {
          if (!raw) return [];
          if (Array.isArray(raw)) return raw as ScheduleEntry[];
          try { return JSON.parse(raw) as ScheduleEntry[]; } catch { return []; }
        };

        /** Merge incoming entries into existing list, keyed by ticket number in description. */
        const mergeEntries = (existing: ScheduleEntry[], incoming: ScheduleEntry[]): ScheduleEntry[] => {
          const incomingMap = new Map<string, ScheduleEntry>();
          const incomingNoKey: ScheduleEntry[] = [];
          for (const e of incoming) {
            const k = extractKey(e.description);
            k ? incomingMap.set(k, e) : incomingNoKey.push(e);
          }

          // Update or keep existing entries
          const result: ScheduleEntry[] = existing.map(e => {
            const k = extractKey(e.description);
            return (k && incomingMap.has(k)) ? incomingMap.get(k)! : e;
          });

          // Add new entries that didn't exist yet
          const existingKeys = new Set(existing.map(e => extractKey(e.description)).filter(Boolean));
          for (const [k, e] of incomingMap) {
            if (!existingKeys.has(k)) result.push(e);
          }
          // Append keyless new entries
          result.push(...incomingNoKey);
          return result;
        };
        ...
        // Fetch existing scheduledUpdates value from Neptune
        const existingQ = `
          PREFIX env: <http://neptune.aws.com/envmgmt/ontology/>
        ...
        const existingRaw: string = existingResult.results?.bindings?.[0]?.val?.value || '[]';

        const existingEntries = parseEntries(existingRaw);
        const incomingEntries = parseEntries(body.scheduledUpdates);
        ...
        const patchQ = `
          ...
          INSERT { <${patchUri}> env:scheduledUpdates "${escapeSparqlPatch(mergedJson)}" }
          WHERE  { OPTIONAL { <${patchUri}> env:scheduledUpdates ?old } }
        `;
        await neptuneClient.executeSparqlUpdate(patchQ);
        result = { patched: patchUri, field: 'scheduledUpdates', total: mergedEntries.length, added: mergedEntries.length - existingEntries.length };
        break;
      }
