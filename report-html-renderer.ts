...
function renderGraph(containerId,data,mode){
  ...
  edgeLabel.append("rect").attr("rx",4).attr("ry",4).attr("width",function(d){var t;if(d.removed){t="-"+d.count}else{t=String(d.count);if(d.newCount>0)t+=" +"+d.newCount;if(d.removedCount>0)t+=" -"+d.removedCount}return Math.max(18,t.length*6+8)}).attr("height",15).attr("fill",function(d){if(d.removed)return"#fee2e2";if(d.removedCount>0&&d.newCount>0)return"#fef9c3";if(d.removedCount>0)return"#fee2e2";return d.newCount>0?"#dcfce7":"#fff"}).attr("stroke",function(d){if(d.removed)return"#dc2626";if(d.removedCount>0&&d.newCount>0)return"#ca8a04";if(d.removedCount>0)return"#dc2626";return d.newCount>0?"#16a34a":"#e2e8f0"}).attr("stroke-width",1);
  var edgeText=edgeLabel.append("text").attr("font-size","8px").attr("font-weight","700").attr("text-anchor","middle").attr("dominant-baseline","central");edgeText.each(function(d){var el=d3.select(this);if(d.removed){el.append("tspan").attr("fill","#991b1b").text("-"+d.count)}else{el.append("tspan").attr("fill","#64748b").text(String(d.count));if(d.newCount>0)el.append("tspan").attr("fill","#16a34a").text(" +"+d.newCount);if(d.removedCount>0)el.append("tspan").attr("fill","#dc2626").text(" -"+d.removedCount)}});
