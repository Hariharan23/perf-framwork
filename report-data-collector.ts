...
export class ReportDataCollector {
  ...
  async collectForOwner(owner: string, reportType: 'weekly' | 'daily' = 'weekly', customRange?: { startDate: string; endDate: string }): Promise<OwnerReportData> {
    ...
    // Step 2: Build per-environment report data in parallel (each uses scoped subgraph)
    const envReportPromises = ownerEnvs.map(env =>
      this.buildEnvironmentReportOptimized(env, periodStart, periodEnd)
    );
  ...
  async collectForEnvironmentIds(environmentIds: string[], reportType: 'weekly' | 'daily' = 'weekly', customRange?: { startDate: string; endDate: string }): Promise<OwnerReportData> {
    ...
    const matchedEnvs = await this.getEnvironmentsByIds(environmentIds);
    ...
    const owners = [...new Set(matchedEnvs.map(e => (e as any).owner || e.properties?.owner).filter(Boolean))];
  ...
  private async getEnvironmentsByOwner(owner: string): Promise<NetworkNode[]> {
    ...
    return allEnvs
      .filter(env => (env.owner || '').toLowerCase() === ownerLower)
      .map(env => this.entityToNetworkNode(env));
  }

  /**
   * Get specific environments by IDs without loading full network graph
   */
  private async getEnvironmentsByIds(environmentIds: string[]): Promise<NetworkNode[]> {
    const ids = Array.from(new Set(environmentIds)).filter(Boolean);
    if (ids.length === 0) return [];

    const entities = await Promise.all(ids.map(id => this.client.getEntityById(id)));
    return entities
      .filter((entity): entity is Entity => !!entity && entity.type === 'Environment')
      .map(entity => this.entityToNetworkNode(entity));
  }

  /**
   * Convert an Entity into the NetworkNode shape expected by report builders
   */
  private entityToNetworkNode(entity: Entity): NetworkNode {
    const { id, name, type, ...rest } = entity as any;
    return {
      id,
      label: name || id,
      type,
  ...
  /**
   * Build report data with environment-scoped network graph instead of full graph
   */
  private async buildEnvironmentReportOptimized(
    envNode: NetworkNode,
    periodStart: Date,
    periodEnd: Date
  ): Promise<EnvironmentReportData> {
    const networkData = await this.client.getNetworkDataForEnvs([envNode.id], 2);
    return this.buildEnvironmentReport(envNode, networkData, periodStart, periodEnd);
  }
  ...
  private countAlerts(environments: EnvironmentReportData[]): number {
    let count = 0;
    for (const env of environments) {
      const hasSLData = !!(env.stats.cpu || env.stats.memory || env.stats.storage || env.stats.healthScore || env.stats.lastSyncedAt);
      if (!hasSLData) continue;
