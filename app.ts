...
import { HistoryRetentionStack } from './cdk-stack/history-retention-stack';
import { CmdbConnectorStack }   from './cdk-stack/cmdb-connector-stack';
import { ServiceNowApiStack }   from './cdk-stack/servicenow-api-stack';
import { CiLinkIntelligenceStack } from './cdk-stack/ci-link-intelligence-stack';
...
const dataPipelineStack = new DataPipelineStack(app, 'SRE-POC-DataPipelineStack', {
  ...
  dataBucketName: neptuneStack.dataBucket.bucketName,
});
dataPipelineStack.addDependency(neptuneStack);
dataPipelineStack.addDependency(pceDiscoveryStack);

// Deploy History Retention Stack (depends on Neptune stack)
const historyRetentionStack = new HistoryRetentionStack(app, 'SRE-POC-HistoryRetentionStack', {
  env: {
    account: process.env.CDK_DEFAULT_ACCOUNT,
    region: process.env.CDK_DEFAULT_REGION || 'us-east-1',
  },
  description: 'EMS History Retention — nightly archive to S3 and Neptune purge with restore capability',
  neptuneClusterEndpoint: neptuneStack.neptuneCluster.attrEndpoint,
  neptuneSecurityGroup:   neptuneStack.neptuneSecurityGroup,
  neptuneVpc:             neptuneStack.vpc,
  neptuneSubnets:         neptuneStack.vpc.privateSubnets,
});
historyRetentionStack.addDependency(neptuneStack);
...
// Deploy CMDB Connector Stack (depends on Neptune stack)
const cmdbConnectorStack = new CmdbConnectorStack(app, 'SRE-POC-CmdbConnectorStack', {
  env: {
    account: process.env.CDK_DEFAULT_ACCOUNT,
    region:  process.env.CDK_DEFAULT_REGION || 'us-east-1',
  },
  description: 'EMS CMDB Connector — ServiceNow CI pull, field mapping, and entity association',
  neptuneClusterEndpoint: neptuneStack.neptuneCluster.attrEndpoint,
  neptuneSecurityGroup:   neptuneStack.neptuneSecurityGroup,
  neptuneVpc:             neptuneStack.vpc,
  neptuneSubnets:         neptuneStack.vpc.privateSubnets,
});
cmdbConnectorStack.addDependency(neptuneStack);

// Deploy ServiceNow API Stack (depends on Neptune + CMDB Connector stacks)
const serviceNowApiStack = new ServiceNowApiStack(app, 'SRE-POC-ServiceNowApiStack', {
  env: {
    account: process.env.CDK_DEFAULT_ACCOUNT,
    region:  process.env.CDK_DEFAULT_REGION || 'us-east-1',
  },
  description: 'EMS ServiceNow Service Map — approval-workflow API for cmdb_rel_ci sync',
  neptuneClusterEndpoint:   neptuneStack.neptuneCluster.attrEndpoint,
  neptuneSecurityGroup:     neptuneStack.neptuneSecurityGroup,
  neptuneVpc:               neptuneStack.vpc,
  neptuneSubnets:           neptuneStack.vpc.privateSubnets,
  cmdbCredentialsSecretArn: cmdbConnectorStack.credentialsSecret.secretArn,
});
serviceNowApiStack.addDependency(neptuneStack);
serviceNowApiStack.addDependency(cmdbConnectorStack);

// Deploy CI Link Intelligence Stack (depends on Neptune + CMDB Connector)
const ciLinkIntelligenceStack = new CiLinkIntelligenceStack(app, 'SRE-POC-CiLinkIntelligenceStack', {
  env: {
    account: process.env.CDK_DEFAULT_ACCOUNT,
    region: process.env.CDK_DEFAULT_REGION || 'us-east-1',
  },
  description: 'SRE-POC CI Link Intelligence — ServiceNow match suggestions and link lifecycle monitoring',
  neptuneClusterEndpoint: neptuneStack.neptuneCluster.attrEndpoint,
  neptuneVpc: neptuneStack.vpc,
  neptuneSecurityGroup: neptuneStack.neptuneSecurityGroup,
  cmdbCredentialsSecretArn: cmdbConnectorStack.credentialsSecret.secretArn,
  cmdbConnectorFunctionArn: cmdbConnectorStack.cmdbConnectorLambda.functionArn,
  aliasTableName: pceDiscoveryStack.aliasTable.tableName,
  aliasTableArn: pceDiscoveryStack.aliasTable.tableArn,
});
ciLinkIntelligenceStack.addDependency(neptuneStack);
ciLinkIntelligenceStack.addDependency(cmdbConnectorStack);
ciLinkIntelligenceStack.addDependency(pceDiscoveryStack);
