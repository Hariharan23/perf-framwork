  ...
  try {
    ...
    switch (operation.toLowerCase()) {
      ...
      case 'get-entity': {
        ...
        // Extract meta_* keys into a separate customMetadata object (prefix stripped)
        const cleanEntity: Record<string, any> = {};
        const customMetadata: Record<string, string> = {};
        for (const [k, v] of Object.entries(rawEntity)) {
          if (k.startsWith('config_')) continue;
          if (k.startsWith('meta_')) { customMetadata[k.slice('meta_'.length)] = String(v ?? ''); continue; }
          cleanEntity[k] = v;
        }

        result = { entity: cleanEntity, configurations, customMetadata };
        break;
      }
