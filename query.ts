...
const ACTION_BADGE: Record<string, string> = {
  ...
  'orphaned-by-retarget':       'disconnected by retarget',
};
...
function buildDisplayChanges(action: string, changes: Record<string, any>): DisplayChange[] {
  ...
  if (action === 'orphaned-by-retarget') {
    if (changes.integrationName)          out.push({ field: 'Integration',          value: changes.integrationName });
    if (changes.previousSourceEnvironment) out.push({ field: 'Previous source',     value: changes.previousSourceEnvironment });
    if (changes.newTarget)                 out.push({ field: 'Retargeted to',       value: changes.newTarget });
    return out;
  }
...
function buildSummary(action: string, changes: Record<string, any>, by: string, entityName?: string): string {
  ...
  if (action === 'orphaned-by-retarget') {
    const int = c.integrationName || 'an integration';
    const src = c.previousSourceEnvironment || 'a source environment';
    const to  = c.newTarget || 'another node';
    return `Became orphan — integration '${int}' from '${src}' was retargeted to '${to}'`;
  }
