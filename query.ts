  ...
  try {
    ...
    switch (operation.toLowerCase()) {
      ...
      case 'get-network-overview': {
        ...
        // Path: /ems/pipelines/{sourceType}/{envId}  →  extract envId from each path
        const pipelineEnvIds = new Set<string>();
        try {
          ...
          do {
            ...
            for (const param of res.Parameters || []) {
              // Name format: /ems/pipelines/{sourceType}/{envId}
              const parts = (param.Name || '').split('/');
              if (parts.length >= 5) {
                pipelineEnvIds.add(parts[4]); // envId is the 5th segment
              }
            }
        ...
        // Stamp hasPipeline on matching nodes
        if (pipelineEnvIds.size > 0) {
          for (const node of overview.nodes) {
            if (pipelineEnvIds.has(node.id)) {
