...
function buildSummary(action: string, changes: Record<string, any>, by: string): string {
  if (!changes || typeof changes !== 'object') {
    return `${capitalize(action)} by '${by}'`;
  }

  if (action === 'create-integration') {
    const int  = changes.integrationName || 'unknown';
    const tgt  = changes.targetEnvName   || 'unknown';
    return `Integration '${int}' linked to '${tgt}'`;
  }
  if (action === 'delete-integration') {
    const int  = changes.integrationName || 'unknown';
    const tgt  = changes.targetEnvName   || 'unknown';
    return `Integration '${int}' unlinked from '${tgt}'`;
  }

  if (action === 'created') {
    const type    = changes.type || 'Entity';
    const cfgKeys = changes.configurations ? Object.keys(changes.configurations) : [];
    const cfgPart = cfgKeys.length > 0
      ? ` with ${cfgKeys.length} configuration${cfgKeys.length > 1 ? 's' : ''} (${cfgKeys.slice(0, 3).join(', ')}${cfgKeys.length > 3 ? ', …' : ''})`
      : '';
    return `${type} created by '${by}'${cfgPart}`;
  }

  if (action === 'deleted') {
    return `Entity deleted by '${by}'`;
  }

  // updated — field-aware natural language
  const parts: string[] = [];
  for (const [field, val] of Object.entries(changes)) {
    ...
    if (field === 'configurations' && typeof val === 'object' && val !== null) {
      ...
      const changed = entries.filter(([, v]) => typeof v === 'object' && v !== null && (v as any).old !== null && 'new' in (v as any));
      const allKeys = entries.map(([k]) => k);
      const preview = allKeys.slice(0, 3).join(', ') + (allKeys.length > 3 ? ', …' : '');

      if (changed.length > 0 || added.length > 0) {
        const cp: string[] = [];
        if (changed.length > 0) cp.push(`${changed.length} key${changed.length > 1 ? 's' : ''} changed`);
        if (added.length > 0)   cp.push(`${added.length} key${added.length > 1 ? 's' : ''} added`);
        parts.push(`Configuration updated — ${cp.join(', ')} (${preview})`);
      } else if (entries.length > 0) {
        parts.push(`${entries.length} configuration${entries.length > 1 ? 's' : ''} updated (${preview})`);
      }
      continue;
    }

    if (typeof val === 'object' && val !== null && 'old' in val && 'new' in val) {
      parts.push(`${label} changed from '${(val as any).old}' to '${(val as any).new}'`);
      continue;
    }

    switch (field) {
      case 'status':           parts.push(`Status changed to '${val}'`); break;
      case 'owner':            parts.push(`Ownership transferred to '${val}'`); break;
      case 'description':      parts.push(`Description updated`); break;
      case 'name':             parts.push(`Renamed to '${val}'`); break;
      case 'primaryUsage':     parts.push(`Primary usage updated to '${val}'`); break;
      case 'currentUsage':     parts.push(`Current usage updated to '${val}'`); break;
      case 'collaborators':    parts.push(`Collaborators updated`); break;
      case 'currentBuild':     parts.push(`Build version changed to '${val}'`); break;
      case 'scheduledUpdates': parts.push(`Scheduled updates modified`); break;
      case 'healthScore':      parts.push(`Health score updated to ${val}`); break;
      case 'currentState':     parts.push(`State changed to '${val}'`); break;
      case 'availability':     parts.push(`Availability updated to ${val}%`); break;
      case 'latency':          parts.push(`Latency updated to ${val}ms`); break;
      case 'cpu':              parts.push(`CPU usage updated to ${val}%`); break;
      case 'memory':           parts.push(`Memory usage updated to ${val}%`); break;
      case 'storage':          parts.push(`Storage usage updated to ${val}%`); break;
      default:                 parts.push(`${label} updated to '${val}'`);
    }
  }
  return parts.length > 0 ? parts.join('; ') : `Updated by '${by}'`;
}
...
function enrichEntry(entry: any): any {
  ...
  return {
    ...
    summary:            buildSummary(action, changes, by),
