  ...
  try {
    ...
    switch (operation.toLowerCase()) {
      ...
      case 'get-environment-detail': {
        ...
        // Extract meta_* keys into customMetadata array and remove from rawFocus
        const focusCustomMetadata: Array<Record<string, string>> = [];
        for (const key of Object.keys(rawFocus)) {
          if (!key.startsWith('meta_')) continue;
          focusCustomMetadata.push({ [key.slice('meta_'.length)]: String(rawFocus[key] ?? '') });
          delete rawFocus[key];
        }
        ...
        // Attach customMetadata to properties (or directly on focus if properties absent)
        if (rawFocus.properties && typeof rawFocus.properties === 'object') {
          (rawFocus.properties as Record<string, any>).customMetadata = focusCustomMetadata;
        } else {
          rawFocus.customMetadata = focusCustomMetadata;
        }
