    ...
    switch (operation.toLowerCase()) {
      ...
      case 'get-entity': {
        ...
        for (const key of Object.keys(rawEntity)) {
          ...
          if (key.startsWith('config_tag_'))  { _tag[key.slice('config_tag_'.length)]   = rawEntity[key]; continue; }
          _cfgKeys.push(key);
        }

        const configurations = _cfgKeys
          .map((key) => {
            ...
            return {
              ...
              configTag:                _tag[cfgName] || '',
              businessPurpose:          _bpVal[cfgName] || '',
              businessPurposeUpdatedBy: _bpBy[cfgName]  || '',
              businessPurposeUpdatedOn: _bpOn[cfgName]  || '',
            };
          })
          .filter((c) => c.configType === 'url');

        // Strip config_* keys; collect meta_* into customMetadata array (prefix stripped)
        const cleanEntity: Record<string, any> = {};
        const customMetadataArr: Array<Record<string, string>> = [];
        for (const [k, v] of Object.entries(rawEntity)) {
          if (k.startsWith('config_')) continue;
          if (k.startsWith('meta_')) {
            customMetadataArr.push({ [k.slice('meta_'.length)]: String(v ?? '') });
            continue;
          }
          cleanEntity[k] = v;
        }
        cleanEntity.customMetadata = customMetadataArr;
      ...
      case 'get-environment-detail': {
        ...
        for (const key of Object.keys(rawFocus)) {
          ...
          if (key.startsWith('config_tag_'))     { tagMap[key.slice('config_tag_'.length)] = raw; delete rawFocus[key]; continue; }
        ...
        const configurationsNew = cfgEntries
          .map(({ name: cfgName, value: cfgValue }) => ({
            configName:               cfgName,
            configValue:              cfgValue,
            configType:               /^https?:\/\/|^ftp:\/\//i.test(cfgValue) ? 'url' : 'text',
            configTag:                tagMap[cfgName] || '',
        ...
        // Hoist status, currentState, environmentType to root of focus node
        rawFocus.status          = rawFocus.properties?.status          ?? null;
        rawFocus.currentState    = rawFocus.stats?.currentState         ?? rawFocus.properties?.currentState ?? null;
        rawFocus.environmentType = rawFocus.properties?.environmentType ?? null;

        // Hoist the same three fields to root of each neighbor node
        for (const neighbor of detail.neighbors as Record<string, any>[]) {
          neighbor.status          = neighbor.properties?.status          ?? null;
          neighbor.currentState    = neighbor.stats?.currentState         ?? neighbor.properties?.currentState ?? null;
          neighbor.environmentType = neighbor.properties?.environmentType ?? null;
        }
