...
import { loadTagMapping } from './url-config-tag-loader';
...
export class IntegrationBuilder {
  ...
  async buildForEnvironment(input: IntegrationBuildInput): Promise<DiscoveryResult> {
    ...
    // Non-URL configs are NOT stored in Neptune (may contain sensitive data).
    // Only URL-type properties are persisted as config_ triples.

    // Step 2b: Store URL properties as config on the source environment
    // so they appear in configurationsNew (used for display + business purpose annotation)
    if (input.urlProperties.length > 0) {
      const urlConfigs: Record<string, string> = {};
      for (const prop of input.urlProperties) {
        urlConfigs[prop.propertyKey] = prop.propertyValue;
      }
      try {
        await this.neptuneClient.addConfigurationProperties(sourceEnvId, urlConfigs);
        console.log(`Stored ${input.urlProperties.length} URL configs on environment ${sourceEnvId}`);
      } catch (err) {
        console.error(`Failed to store URL configs:`, err);
        result.errors.push({ key: '_url_configs', error: String(err) });
      }
    }

    // Step 2c: Apply application tags from the S3 tag-mapping property file.
    // Stores config_tag_<key> = "ApplicationName" triples for matching URL config keys.
    if (input.urlProperties.length > 0) {
      try {
        const tagMapping = await loadTagMapping(process.env.S3_BUCKET || '');
        if (tagMapping.size > 0) {
          const tagConfigs: Record<string, string> = {};
          for (const prop of input.urlProperties) {
            const tag = tagMapping.get(prop.propertyKey.toLowerCase());
            if (tag) tagConfigs[`tag_${prop.propertyKey}`] = tag;
          }
          if (Object.keys(tagConfigs).length > 0) {
            await this.neptuneClient.addConfigurationProperties(sourceEnvId, tagConfigs);
            console.log(`Applied ${Object.keys(tagConfigs).length} config tags on environment ${sourceEnvId}`);
          }
        }
      } catch (err) {
        console.error(`Failed to apply config tags (non-fatal):`, err);
        // Non-fatal — tagging failure must not break the pipeline
      }
    }
