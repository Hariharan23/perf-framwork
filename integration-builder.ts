...
export interface DiscoveryResult {
  ...
  pendingIntegrationsCreated: string[]; // Unresolvable hostnames → stored as PendingIntegration
  ...
  resolvedByAutoCreate: number;    // Reserved — was auto-create of orphan nodes; now always 0 (unresolved → PendingIntegration)
...
export class IntegrationBuilder {
  ...
  private async processUrlProperty(
    ...
    if (resolved.isNewEnvironment) {
      // Do NOT create an orphan Environment node.  Instead, store a
      // PendingIntegration record that preserves ALL original property
      // details so the Integration can be properly created when the
      // operator manually supplies the canonical target environment name.
      const parsedKeyMeta = parsePropertyKey(prop.propertyKey);
      const pending = await this.neptuneClient.createPendingIntegration({
      ...
      if (!pending.alreadyExisted) {
        result.pendingIntegrationsCreated.push(`${sourceEnvName}.${prop.propertyKey}→${prop.parsedUrl.hostname}`);
        result.errors.push({ key: prop.propertyKey, error: `Hostname "${prop.parsedUrl.hostname}" could not be resolved; saved as PendingIntegration (${pending.id})` });
      }
      // Return null — Integration + relationships are deferred until manual connect
      return null;
    }
