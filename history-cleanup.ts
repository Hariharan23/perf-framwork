...
async function uploadNdjsonGz(key: string, lines: string[], recordCount: number): Promise<void> {
  ...
  input.pipe(gz).pipe(collector);
  await pipeline(input, gz, collector);
...
async function deleteHistoryNodes(records: HistoryRecord[]): Promise<number> {
  // Collect all history node URIs by ID, then delete all triples per node
    ...
    const deleteQuery = `
      ...
      WHERE {
        ?h env:type "History" ;
           env:id ?id .
        FILTER (?id IN (${values}))
        OPTIONAL { ?h env:type ?type }
        OPTIONAL { ?h env:entityId ?eid }
        OPTIONAL { ?h env:entityName ?ename }
        OPTIONAL { ?h env:action ?action }
        OPTIONAL { ?h env:timestamp ?ts }
        OPTIONAL { ?h env:user ?user }
        OPTIONAL { ?h env:changes ?changes }
      }
    `;
