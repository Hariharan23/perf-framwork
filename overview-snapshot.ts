...
function parseJsonScheduleWindows(schedule: string): Array<{ windowStart: string; windowEnd: string; description: string }> {
  try {
    ...
    for (const w of arr) {
      ...
      const endDate: string     = w.endDate     || '';
      ...
      results.push({
        ...
        windowEnd:   `${windowEndDate}T${endTime}:00Z`,
        description,
      });
    }
    return results;
...
function computeDerivedData(snap: any): any {
  ...
  function classifyEnvStatus(envId: string, nativeStatusRaw: string): 'Critical' | 'Major' | 'Warning' | 'Healthy' {
  ...
  const buckets: Record<string, number> = { Critical: 0, Major: 0, Warning: 0, Healthy: 0 };
