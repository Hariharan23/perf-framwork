...
function parseJsonScheduleWindows(schedule: string): Array<{ windowStart: string; windowEnd: string; description: string }> {
  try {
    ...
    // Use today in UTC as a fallback for entries that have no date yet
    const todayUtc = new Date().toISOString().slice(0, 10);
    for (const w of arr) {
      const date: string        = (w.date        || '').trim();
      const endDate: string     = (w.endDate     || '').trim();
      const startTime: string   = (w.startTime   || '').trim();
      const endTime: string     = (w.endTime     || startTime || '00:00').trim();
      const description: string = w.description || '';

      // Use fallback date/time for entries that have no window yet (e.g. changes in planning)
      const effectiveDate      = date      || todayUtc;
      const effectiveStartTime = startTime || '00:00';

      // If an explicit endDate is provided use it; otherwise derive from time cross-midnight
      let windowEndDate: string;
      if (endDate) {
        windowEndDate = endDate;
      } else {
        const [year, month, day] = effectiveDate.split('-').map(Number);
        const startH = parseInt(effectiveStartTime.split(':')[0], 10);
        const effectiveEndTime = endTime || effectiveStartTime;
        const endH   = parseInt(effectiveEndTime.split(':')[0], 10);
        const endDay = endH < startH ? day + 1 : day;
        windowEndDate = `${year}-${String(month).padStart(2,'0')}-${String(endDay).padStart(2,'0')}`;
      }

      const effectiveEndTime = endTime || effectiveStartTime;
      results.push({
        windowStart: `${effectiveDate}T${effectiveStartTime}:00Z`,
        windowEnd:   `${windowEndDate}T${effectiveEndTime}:00Z`,
        description,
      });
    }
    return results;
...
function computeDerivedData(snap: any): any {
  ...
  // Uses the same classification logic as the Visualize page getNodeHealthState():
  //   1. Neptune native status/currentState (critical/major/warning thresholds)
  //   2. Performance stats: healthScore, cpu, memory, storage, availability
  // Buckets: Critical | Major | Warning | Healthy  (matches visualize health chips)
  ...
  // Build stats lookup from healthEnts (which carry the full ScienceLogic stat object)
  const statsById = new Map<string, any>();
  for (const e of healthEnts) {
    if (e.id && e.stats) statsById.set(String(e.id), e.stats);
  }

  function classifyEnvStatus(envId: string, nativeStatusRaw: string): 'Critical' | 'Major' | 'Warning' | 'Healthy' | 'Unknown' {
    // Step 1 — native Neptune status field (same as visualize page)
    const ns = nativeStatusRaw.toLowerCase();
    if (['critical', 'down', 'error', 'unreachable'].includes(ns)) return 'Critical';
    if (ns === 'major') return 'Major';
    if (['warning', 'degraded', 'inactive'].includes(ns)) return 'Warning';

    // Step 2 — performance stats thresholds (mirror of getNodeHealthState)
    const s = statsById.get(envId);
    if (!s) return 'Unknown';  // no stats record at all — not monitored
    const hasSLData = !!(s.currentState || s.cpu || s.memory || s.storage ||
      s.availability || s.latency || (s.healthScore && parseFloat(s.healthScore) > 0) || s.lastSyncedAt);
    if (!hasSLData) return 'Unknown';  // stats object present but empty — not monitored

    const state = (s.currentState || '').toLowerCase();
    if (['critical', 'down', 'error', 'unreachable'].includes(state)) return 'Critical';
    if (state === 'major') return 'Major';
    if (['warning', 'degraded'].includes(state)) return 'Warning';

    const hs = parseFloat(s.healthScore);
    if (!isNaN(hs)) {
      if (hs < 40) return 'Critical';
      if (hs < 60) return 'Major';
      if (hs < 75) return 'Warning';
    }
    const cpu = parseFloat(s.cpu);
    const mem = parseFloat(s.memory);
    const sto = parseFloat(s.storage);
    if (cpu > 95 || mem > 95 || sto > 95) return 'Critical';
    if (cpu > 85 || mem > 85 || sto > 85) return 'Warning';

    const avail = parseFloat(s.availability);
    if (!isNaN(avail) && avail < 95) return 'Critical';
    if (!isNaN(avail) && avail < 99) return 'Warning';

    return 'Healthy';
  }

  const buckets: Record<string, number> = { Critical: 0, Major: 0, Warning: 0, Healthy: 0, Unknown: 0 };
  for (const env of envs) {
    const id = String(env.id || '');
    const nativeRaw = nodeStatusById.get(id) || (env.status as string) || '';
    const key = classifyEnvStatus(id, nativeRaw);
    buckets[key]++;
  }
  ...
  // Score is derived from three checks — 100% only when all pass:
  //   Owner coverage      40 pts — every environment must have an owner assigned
  //   Orphan-free         40 pts — no unresolved orphan environments
  //   Monitoring coverage 20 pts — every environment is monitored (has health stats)
  const total     = envs.length;
  const withOwner = envs.filter((e: any) => e.owner && String(e.owner).trim() !== '').length;

  // Owner check (40 pts): proportional to fraction of envs with an owner
  const ownerPts    = total === 0 ? 40 : Math.round((withOwner / total) * 40);
  const ownerStatus = withOwner === total ? 'pass' : withOwner > total / 2 ? 'partial' : 'fail';

  // Orphan check (40 pts): proportional to fraction of envs that are NOT orphans
  const orphanPts = total === 0 ? 40 : Math.round(((total - orphans.length) / total) * 40);

  // Monitoring check (20 pts): proportional to fraction of envs with health stats
  const monitoredCount  = healthEnts.filter((e: any) => e.type === 'Environment').length;
  const monitoredPts    = total === 0 ? 20 : Math.round((monitoredCount / total) * 20);
  const monitoredStatus = monitoredCount === total ? 'pass' : monitoredCount > total / 2 ? 'partial' : 'fail';

  const govScore      = ownerPts + orphanPts + monitoredPts;
  ...
  const governanceScore = {
    ...
    checks: [
      ...
      {
        source: 'monitoredCheck',
        count:  monitoredCount,
        total,
        status: monitoredStatus,
        label:  `${monitoredCount} / ${total}`,
      },
      {
        source: 'aliasCheck',
        count:  aliases.length,
        pass:   aliases.length > 0,
