...
async function collectScheduledUpdates(): Promise<any[]> {
  // Two-query approach — avoids a single SPARQL join that can silently return
  // zero rows when rdf:type and env:scheduledUpdates triples don't share the
  // exact same canonical subject node.
  //
  // Query A: all environments (same rdf:type pattern as getEntitiesByType —
  //          proven to return results).
  // Query B: all scheduledUpdates values, indexed by env:id.
  // Then join in-process on envId.
  ...
  const envsQuery = `
    ...
    PREFIX env: <${ONTOLOGY_PREFIX}>
    SELECT ?id ?name ?owner ?collaborators WHERE {
  ...
  const schedQuery = `
    PREFIX env: <${ONTOLOGY_PREFIX}>
    SELECT ?id ?scheduledUpdates WHERE {
      ?entity env:id ?id ;
  ...
  console.log('collectScheduledUpdates: running environment + schedule queries');
  const [envsResult, schedResult] = await Promise.all([
    neptune.executeSparqlQuery(envsQuery),
    neptune.executeSparqlQuery(schedQuery),
  ]);

  const envCount  = envsResult.results?.bindings?.length ?? 0;
  const schedCount = schedResult.results?.bindings?.length ?? 0;
  console.log(`collectScheduledUpdates: ${envCount} environments, ${schedCount} schedule bindings`);

  // Build a name/owner/collaborators lookup keyed on envId
  const envMap = new Map<string, { envName: string; owner: string; collaborators: string }>();
  for (const b of (envsResult.results?.bindings || [])) {
    const id = b.id?.value || '';
    if (id) {
      envMap.set(id, {
        envName:       b.name?.value          || '',
        owner:         b.owner?.value         || '',
  ...
  // Join schedule bindings with env metadata
  const items = (schedResult.results?.bindings || [])
    .map((b: any) => {
      const envId   = b.id?.value              || '';
      const schedule = b.scheduledUpdates?.value || '';
      if (!envId || !schedule) return null;
      const meta = envMap.get(envId) || { envName: envId, owner: '', collaborators: '' };
      return { envId, schedule, ...meta };
    })
    .filter(Boolean) as { envId: string; envName: string; owner: string; collaborators: string; schedule: string }[];

  console.log(`collectScheduledUpdates: ${items.length} items after join`);
