...
function computeDerivedData(snap: any): any {
  ...
  // Two classification sources so topology flags stay consistent with the alert panel:
  //   A) activeAlerts — stats-based thresholds (cpu, memory, latency, healthScore)
  //   B) raw Neptune node state fields — currentState / status in top-level or properties
  ...
  for (const alert of activeAlerts) {
    if (alert.severity === 'critical') {
      criticalIds.add(alert.entityId);
      warningIds.delete(alert.entityId);        // resolve any prior warning entry
    } else if (!criticalIds.has(alert.entityId)) {
      warningIds.add(alert.entityId);
    }
  }

  // Source B: Neptune topology node state fields
  // getNetworkOverview() may store status in node.properties.status or node.status
  for (const node of netNodes) {
    const stateRaw = (
      node.currentState             ||
      node.properties?.currentState ||
      node.status                   ||
      node.properties?.status       || ''
    ).toLowerCase();
    if (stateRaw === 'critical') {
      criticalIds.add(node.id);
      warningIds.delete(node.id);               // escalate: warning → critical
    } else if ((stateRaw === 'warning' || stateRaw === 'degraded') && !criticalIds.has(node.id)) {
      warningIds.add(node.id);
    }
  }

  const alertNodeIds = new Set<string>([...criticalIds, ...warningIds]);
  const hasNodes     = netNodes.length > 0;

  // Expand to one-hop neighbours of every alert node
  ...
  for (const edge of netEdges) {
    ...
    if (alertNodeIds.has(src) || alertNodeIds.has(tgt)) {
      if (src) visibleIds.add(src);
      if (tgt) visibleIds.add(tgt);
    }
  }

  // allHealthy is only true when there IS topology data and none are alerted
  const allHealthy     = hasNodes && visibleIds.size === 0;
  const noData         = !hasNodes;
  const neighbourCount = visibleIds.size - alertNodeIds.size;

  // When allHealthy or no data, pass all nodes/edges so the frontend can
  // render a full-graph view (or a placeholder) without an empty payload.
  const filterToAlerts = !allHealthy && !noData;
  const topologyFocus = {
    allHealthy,
    noData,
    criticalCount:  criticalIds.size,
    warnCount:      warningIds.size,
    neighbourCount,
    badgeText: noData
      ? '⚠ No topology data available'
      : allHealthy
        ? '✓ All environments are healthy'
        : `${criticalIds.size} critical · ${warningIds.size} warning · ${neighbourCount} connected`,
    nodes: (filterToAlerts ? netNodes.filter((n: any) => visibleIds.has(n.id)) : netNodes)
      .map((n: any) => ({
        ...n,
        _isCritical: criticalIds.has(n.id),
        _isWarning:  warningIds.has(n.id),
      })),
    edges: filterToAlerts
      ? netEdges.filter((e: any) => visibleIds.has(e.source) && visibleIds.has(e.target))
      : netEdges,
  };
