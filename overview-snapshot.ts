...
function computeDerivedData(snap: any): any {
  ...
  for (const node of netNodes) {
    const isCrit = node.currentState === 'critical' || node.status === 'critical';
    const isWarn = !isCrit && (node.currentState === 'warning' ||
                               node.status === 'warning' || node.status === 'degraded');
    if (isCrit) criticalIds.add(node.id);
    else if (isWarn) warningIds.add(node.id);
  }
  ...
  for (const edge of netEdges) {
    if (alertNodeIds.has(edge.source) || alertNodeIds.has(edge.target)) {
      visibleIds.add(edge.source);
      visibleIds.add(edge.target);
    }
  }
  const allHealthy     = visibleIds.size === 0;
