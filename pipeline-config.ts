...
import { saveTagMapping, loadTagMapping, TAG_FILE_KEY } from '../shared/url-config-tag-loader';
...
const DATA_BUCKET      = process.env.S3_BUCKET || '';
...
export async function handler(event: APIGatewayProxyEvent, _ctx: Context): Promise<any> {
  ...
  try {
    // Support both GET (queryStringParameters) and POST (body)
    ...
    switch (operation) {
      ...
      // ── URL config tag mapping ────────────────────────────────────────────

      case 'upload-tag-mapping': {
        if (!body.content || typeof body.content !== 'string') {
          return fail(400, 'Missing "content" field — send the raw .properties file text');
        }
        if (!DATA_BUCKET) return fail(500, 'S3_BUCKET is not configured on this Lambda');
        await saveTagMapping(body.content, DATA_BUCKET, REGION);
        // Parse and return summary so caller can verify what was loaded
        const { parseTagProperties } = await import('../shared/url-config-tag-loader');
        const mapping = parseTagProperties(body.content);
        const summary: Record<string, string[]> = {};
        for (const [key, app] of mapping.entries()) {
          if (!summary[app]) summary[app] = [];
          summary[app].push(key);
        }
        return ok({ message: 'Tag mapping saved', s3Key: TAG_FILE_KEY, totalKeys: mapping.size, summary });
      }

      case 'get-tag-mapping': {
        if (!DATA_BUCKET) return fail(500, 'S3_BUCKET is not configured on this Lambda');
        const mapping = await loadTagMapping(DATA_BUCKET, REGION);
        const summary: Record<string, string[]> = {};
        for (const [key, app] of mapping.entries()) {
          if (!summary[app]) summary[app] = [];
          summary[app].push(key);
        }
        return ok({ totalKeys: mapping.size, summary });
      }
...
/** Partially mask a sensitive string, showing only the last 4 characters */
...
function maskSensitive(config: any): any {
  const out = { ...config };
  if (out.password)     out.password     = partialMask(String(out.password));
  if (out.clientId)     out.clientId     = partialMask(String(out.clientId));
  if (out.clientSecret) out.clientSecret = partialMask(String(out.clientSecret));
  return out;
}
...
async function upsertEnvironment(sourceType: string, envId: string, config: any): Promise<any> {
  ...
  // If sensitive fields contain a masked placeholder (starts with ••••), preserve the existing stored values
  const isMasked = (v: any) => typeof v === 'string' && v.startsWith('••••');

  const needsExisting =
    isMasked(finalConfig.password)     ||
    isMasked(finalConfig.clientId)     ||
    isMasked(finalConfig.clientSecret);

  if (needsExisting) {
    const existing = await getFullConfig(sourceType, envId);

    if (isMasked(finalConfig.password)) {
    ...
    if (isMasked(finalConfig.clientId)) {
      if (existing?.clientId) {
        finalConfig.clientId = existing.clientId;
      } else {
        throw new Error('Client ID is required for a new environment');
      }
    }

    if (isMasked(finalConfig.clientSecret)) {
      if (existing?.clientSecret) {
        finalConfig.clientSecret = existing.clientSecret;
      } else {
        throw new Error('Client secret is required for a new environment');
      }
    }

    // Preserve existing schedule metadata (only needs one fetch)
