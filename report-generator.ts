  ...
  try {
    ...
    switch (operation.toLowerCase()) {
      ...
      case 'generate-report': {
        const owner = requestBody.owner || event.queryStringParameters?.owner;
        const qsEnvIds = event.multiValueQueryStringParameters?.environmentIds;
        const environmentIds: string[] | undefined = requestBody.environmentIds || (qsEnvIds && qsEnvIds.length > 0 ? qsEnvIds : undefined);
...
// Uses a lean SPARQL query that returns only distinct owner strings — avoids
// the expensive full-graph getNetworkData() call (all nodes + all properties).
  ...
  const query = `
    ...
    SELECT DISTINCT ?owner WHERE {
      ?entity env:type "Environment" ;
              env:owner ?owner .
      FILTER(BOUND(?owner) && ?owner != "")
    }
    ORDER BY ?owner
  `;
  const result = await neptuneClient.executeSparqlQuery(query);
  return (result.results?.bindings || []).map((b: any) => b.owner?.value).filter(Boolean) as string[];
}
