...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    switch (operation) {
      ...
      case 'cmdb-list-runs': {
        ...
        const runs = (scanResult.Items || [])
          .map(item => ({
            ...
            succeeded: Number(item.succeeded?.N ?? 0),
            failed:    Number(item.failed?.N    ?? 0),
            errors:    JSON.parse(item.errors?.S || '[]'),
          }))
      ...
      default:
        return err(400, `Unknown operation: "${operation}". Valid operations: cmdb-test-connection, cmdb-search, cmdb-get-fields, cmdb-link, cmdb-refresh, cmdb-update-fields, cmdb-unlink, cmdb-get-status, cmdb-set-mapping, cmdb-get-mapping, cmdb-list-linked, cmdb-get-scheduler-config, cmdb-set-scheduler-config, cmdb-trigger-refresh-now, cmdb-list-runs`);
    }
