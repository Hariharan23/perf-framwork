...
export interface EnvNode {
  ...
  sourceEnvName?: string;   // set when renamed/merged by the cleanser (alias)
  hasPipeline?: boolean;    // true when this env is the primary target of a pipeline config (set in query.ts)
}
...
${deleteWherePatterns}      }
  ...
  async getNetworkOverview(): Promise<NetworkOverview> {
    const envsQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?id ?name ?status ?sourceService ?sourceEnvName WHERE {
        ?entity env:id ?id ;
                env:name ?name ;
                env:type "Environment" .
        OPTIONAL { ?entity env:status ?status }
        OPTIONAL { ?entity env:sourceService ?sourceService }
        OPTIONAL { ?entity env:config_sourceEnvName ?sourceEnvName }
      }
    `;
    ...
    const nodes: EnvNode[] = (envsResult.results?.bindings || []).map((b: any) => ({
      ...
      sourceEnvName: b.sourceEnvName?.value || undefined,
    }));
