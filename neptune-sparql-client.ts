...
export class NeptuneSparqlClient {
  ...
  /**
   * Check whether a Neptune Environment node (by entity ID) has any Relationship
   * triples pointing to or from it. Used to detect nodes that have become edgeless
   * after stale-integration cleanup.
   */
  async hasAnyEdges(entityId: string): Promise<boolean> {
    const query = `
      PREFIX env: <${this.ontologyPrefix}>
      ASK {
        ?rel env:type "Relationship" .
  ...
  /**
   * Hard-delete all triples for a given entity ID (used to remove auto-created
   * Environment nodes that have become fully edgeless).
   */
  async deleteEnvironmentNode(entityId: string): Promise<void> {
    const deleteQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      DELETE { ?e ?p ?o }
      WHERE  { ?e env:id "${this.escapeSparql(entityId)}" ; ?p ?o }
    `;
    await this.executeSparqlUpdate(deleteQuery);
    console.log(`[neptune] deleted edgeless auto-created environment node ${entityId}`);
  }

  /**
   * Return all Environment nodes that have zero Relationship edges.
   * Includes auto-create stamped metadata (rawHostname, createdByRun, etc.)
   * so the frontend can attribute each orphan to the pipeline run that created it.
   */
  async listOrphanNodes(): Promise<Array<{
    id: string;
    name: string;
    ...
    const query = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?id ?name ?rawHostname ?createdByRun ?createdBySource ?createdAt
             ?propertyKey ?fullUrl ?protocol ?port ?path ?endpointHint ?sourceFile WHERE {
        ?e env:type "Environment" ;
           env:id   ?id ;
           env:name ?name .
        OPTIONAL { ?e env:config_rawHostname     ?rawHostname }
        OPTIONAL { ?e env:config_createdByRun    ?createdByRun }
        OPTIONAL { ?e env:config_createdBySource ?createdBySource }
        OPTIONAL { ?e env:config_createdAt       ?createdAt }
        OPTIONAL { ?e env:config_propertyKey     ?propertyKey }
        OPTIONAL { ?e env:config_fullUrl         ?fullUrl }
        OPTIONAL { ?e env:config_protocol        ?protocol }
        OPTIONAL { ?e env:config_port            ?port }
        OPTIONAL { ?e env:config_path            ?path }
        OPTIONAL { ?e env:config_endpointHint    ?endpointHint }
        OPTIONAL { ?e env:config_sourceFile      ?sourceFile }
        FILTER NOT EXISTS {
          { ?rel env:sourceEntityId ?id }
          UNION
          { ?rel env:targetEntityId ?id }
        }
      }
      ORDER BY ?name
    `;
    ...
    return (result.results?.bindings || []).map((b: any) => ({
      id:              b.id?.value || '',
      name:            b.name?.value || '',
      rawHostname:     b.rawHostname?.value  || null,
      createdByRun:    b.createdByRun?.value || null,
      createdBySource: b.createdBySource?.value || null,
      createdAt:       b.createdAt?.value    || null,
      propertyKey:     b.propertyKey?.value  || null,
      fullUrl:         b.fullUrl?.value      || null,
      protocol:        b.protocol?.value     || null,
      port:            b.port?.value         || null,
      path:            b.path?.value         || null,
      endpointHint:    b.endpointHint?.value || null,
      sourceFile:      b.sourceFile?.value   || null,
    }));
  }

  // ─── Pending Integrations ────────────────────────────────────────────────
  // When PCE discovery encounters a hostname it cannot resolve to a known
  // environment, it creates a PendingIntegration record (instead of an orphan
  // Environment node) that preserves ALL original property details.  The full
  // Integration entity + target Environment are created only when the operator
  // manually supplies the canonical target environment name.

  async createPendingIntegration(data: {
    ...
    // Idempotency: one pending record per sourceEnvironment + propertyKey
    const dupCheck = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?id WHERE {
        ?e env:type "PendingIntegration" ;
           env:id   ?id ;
           env:config_sourceEnvironment "${this.escapeSparql(data.sourceEnvironment)}" ;
           env:config_propertyKey       "${this.escapeSparql(data.propertyKey)}" .
      }
      LIMIT 1
    `;
    const dupResult = await this.executeSparqlQuery(dupCheck);
    const existingId = dupResult.results?.bindings?.[0]?.id?.value;
    if (existingId) {
    ...
    const esc = (v: string) => this.escapeSparql(v);

    const props = [
      `env:type                        "PendingIntegration"`,
      `env:id                          "${esc(id)}"`,
      `env:config_sourceEnvironment    "${esc(data.sourceEnvironment)}"`,
      `env:config_sourceEnvironmentId  "${esc(data.sourceEnvironmentId)}"`,
      `env:config_propertyKey          "${esc(data.propertyKey)}"`,
      `env:config_fullUrl              "${esc(data.fullUrl)}"`,
      `env:config_rawHostname          "${esc(data.rawHostname)}"`,
      `env:config_protocol             "${esc(data.protocol)}"`,
      `env:config_port                 "${esc(data.port)}"`,
      `env:config_path                 "${esc(data.path)}"`,
      `env:config_endpointHint         "${esc(data.endpointHint)}"`,
      `env:config_createdByRun         "${esc(data.createdByRun)}"`,
      `env:config_createdAt            "${esc(createdAt)}"`,
      ...(data.sourceFile ? [`env:config_sourceFile "${esc(data.sourceFile)}"`] : []),
    ].join(' ;\n                  ');
    ...
    `;
    await this.executeSparqlUpdate(update);
    console.log(`[neptune] created PendingIntegration ${id}: ${data.sourceEnvironment}.${data.propertyKey} → ${data.rawHostname}`);
    return { id, alreadyExisted: false };
  }

  async listPendingIntegrations(): Promise<Array<{
    id:                  string;
    sourceEnvironment:   string;
    sourceEnvironmentId: string;
    propertyKey:         string;
    ...
    const query = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?id ?sourceEnvironment ?sourceEnvironmentId ?propertyKey ?fullUrl
             ?rawHostname ?protocol ?port ?path ?endpointHint ?sourceFile
             ?createdByRun ?createdAt WHERE {
        ?e env:type "PendingIntegration" ;
           env:id   ?id .
        OPTIONAL { ?e env:config_sourceEnvironment    ?sourceEnvironment }
        OPTIONAL { ?e env:config_sourceEnvironmentId  ?sourceEnvironmentId }
        OPTIONAL { ?e env:config_propertyKey          ?propertyKey }
        OPTIONAL { ?e env:config_fullUrl              ?fullUrl }
        OPTIONAL { ?e env:config_rawHostname          ?rawHostname }
        OPTIONAL { ?e env:config_protocol             ?protocol }
      ...
      ORDER BY DESC(?createdAt) ?sourceEnvironment ?propertyKey
    `;
    const result = await this.executeSparqlQuery(query);
    return (result.results?.bindings || []).map((b: any) => ({
      id:                  b.id?.value || '',
      sourceEnvironment:   b.sourceEnvironment?.value   || '',
      sourceEnvironmentId: b.sourceEnvironmentId?.value || '',
      propertyKey:         b.propertyKey?.value         || '',
      fullUrl:             b.fullUrl?.value             || '',
      rawHostname:         b.rawHostname?.value         || '',
      protocol:            b.protocol?.value            || '',
      port:                b.port?.value                || '',
      path:                b.path?.value                || '',
      endpointHint:        b.endpointHint?.value        || '',
      sourceFile:          b.sourceFile?.value          || null,
      createdByRun:        b.createdByRun?.value        || '',
      createdAt:           b.createdAt?.value           || '',
    }));
  }

  async deletePendingIntegration(id: string): Promise<void> {
    const update = `
      PREFIX env: <${this.ontologyPrefix}>
      DELETE { ?e ?p ?o }
      WHERE  { ?e env:id "${this.escapeSparql(id)}" ; env:type "PendingIntegration" ; ?p ?o }
    `;
    await this.executeSparqlUpdate(update);
    console.log(`[neptune] deleted PendingIntegration ${id}`);
  }

  /**
   * Rename an Environment node in Neptune.
   * Updates env:name in-place; all existing Relationship edges remain intact.
   */
  async renameEnvironment(nodeId: string, newName: string): Promise<void> {
    const update = `
      PREFIX env: <${this.ontologyPrefix}>
      DELETE { ?e env:name ?oldName }
      INSERT { ?e env:name "${this.escapeSparql(newName)}" }
      WHERE  { ?e env:id   "${this.escapeSparql(nodeId)}" ; env:name ?oldName }
    `;
    await this.executeSparqlUpdate(update);
    console.log(`[neptune] renamed environment ${nodeId} → "${newName}"`);
  }

  /**
   * Merge fromNode into intoNode:
   *   1. Re-point all Relationship triples that reference fromNode to intoNode.
   *   2. Copy any config_ properties missing on intoNode.
   *   3. Delete fromNode entirely.
   */
  async mergeEnvironments(fromId: string, intoId: string): Promise<void> {
    // 1. Re-point source-side relationships
    const rewireSrc = `
      PREFIX env: <${this.ontologyPrefix}>
      DELETE { ?rel env:sourceEntityId "${this.escapeSparql(fromId)}" }
      INSERT { ?rel env:sourceEntityId "${this.escapeSparql(intoId)}" }
      WHERE  { ?rel env:sourceEntityId "${this.escapeSparql(fromId)}" }
    `;
    await this.executeSparqlUpdate(rewireSrc);

    // 2. Re-point target-side relationships
    const rewireTgt = `
      PREFIX env: <${this.ontologyPrefix}>
      DELETE { ?rel env:targetEntityId "${this.escapeSparql(fromId)}" }
      INSERT { ?rel env:targetEntityId "${this.escapeSparql(intoId)}" }
      WHERE  { ?rel env:targetEntityId "${this.escapeSparql(fromId)}" }
    `;
    await this.executeSparqlUpdate(rewireTgt);

    // 3. Copy config_ properties not already present on intoNode
    const copyConfig = `
      PREFIX env: <${this.ontologyPrefix}>
      INSERT {
        ?into ?p ?v
      ...
      WHERE {
        ?from env:id "${this.escapeSparql(fromId)}" .
        ?into env:id "${this.escapeSparql(intoId)}" .
        ?from ?p ?v .
        FILTER(STRSTARTS(STR(?p), "${this.ontologyPrefix}config_"))
        FILTER NOT EXISTS { ?into ?p ?any }
      }
    `;
    await this.executeSparqlUpdate(copyConfig);

    // 4. Delete the from-node entirely
    await this.deleteEnvironmentNode(fromId);
    console.log(`[neptune] merged environment ${fromId} into ${intoId}`);
  }
