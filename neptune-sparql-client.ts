...
export interface EnvNode {
  ...
  name: string;
  status?: string;
  currentState?: string;    // performance state reported by monitoring (Critical, Warning, Normal)
  ...
  environmentType?: string; // On-Prem or Cloud
}
...
export interface LinkedCmdbEntity {
  entityId:   string;
  entityName: string;
  sysId:      string;
  ciClass:    string;
  ...
  syncStatus: string | null;  // 'ok' | 'error' | 'partial' | null
}
...
export class NeptuneSparqlClient {
   ...
   */
  /**
   * Keys matching these patterns are considered sensitive and will be masked
   * before storage: all but the last 4 characters are replaced with *s.
   * Matching is case-insensitive against the config key name.
   */
  private static readonly SENSITIVE_KEY_PATTERN =
    /password|passwd|secret|token|apikey|api_key|credential|cred|auth|private[_-]?key|access[_-]?key|client[_-]?secret/i;

  private maskSensitiveValue(key: string, value: string): string {
    if (!NeptuneSparqlClient.SENSITIVE_KEY_PATTERN.test(key)) return value;
  ...
  async getEntitiesByType(entityType: string): Promise<Entity[]> {
    const query = `
      ...
      SELECT ?id ?name ?uniqueIdentifier ?owner ?description ?createdAt ?status ?environmentType WHERE {
        ...
        OPTIONAL { ?entity env:environmentType ?environmentType }
      }
    `;
    ...
    return result.results?.bindings?.map((binding: any) => ({
      ...
      environmentType: binding.environmentType?.value,
    })) || [];
  }
   ...
   */
  async getNetworkDataForEnvs(envIds?: string[], maxHops: number = 2): Promise<NetworkData> {
    if (!envIds || envIds.length === 0) {
      return this.getNetworkData();
    }

    // BFS over Environment nodes using the 2-hop hasIntegration/integratesWith pattern.
    // Each "hop" finds Environments directly connected to the current frontier.
    const allEnvIds = new Set<string>(envIds);
    ...
    for (let hop = 0; hop < maxHops && frontier.size > 0; hop++) {
      ...
      const hopQuery = `
        PREFIX env: <${this.ontologyPrefix}>
        SELECT DISTINCT ?envB WHERE {
          VALUES ?envA { ${frontierValues} }
      ...
      (hopResult.results?.bindings || []).forEach((b: any) => {
        const id = b.envB?.value;
        if (id && !allEnvIds.has(id)) {
          newIds.add(id);
          allEnvIds.add(id);
        }
      });
    ...
    // Fetch Environment node details for all discovered envs
    const idValues = Array.from(allEnvIds).map(id => `"${this.escapeSparql(id)}"`).join(' ');

    const nodesQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?id ?name ?status ?sourceService WHERE {
        VALUES ?id { ${idValues} }
        ?entity env:id ?id ;
                env:name ?name ;
    ...
    // Fetch weighted env-to-env edges using the same 2-hop pattern
    const edgesQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?envA ?envB (COUNT(DISTINCT ?integrationId) AS ?weight) WHERE {
        VALUES ?envA { ${idValues} }
        VALUES ?envB { ${idValues} }
        ?relA env:type "Relationship" ;
              env:relationshipType "hasIntegration" ;
              env:sourceEntityId ?envA ;
              env:targetEntityId ?integrationId .
    ...
    const nodes = (nodesResult.results?.bindings || []).map((b: any) => ({
      id: b.id?.value || '',
      name: b.name?.value || '',
      label: b.name?.value || '',
      type: 'Environment',
      status: b.status?.value || undefined,
      sourceService: b.sourceService?.value || undefined,
    }));

    const nodeIds = new Set(nodes.map((n: any) => n.id));
    const edges = (edgesResult.results?.bindings || []).map((b: any) => {
  ...
  async getNetworkData(): Promise<NetworkData> {
    ...
    const nodesQuery = `
      ...
      SELECT ?id ?name ?type ?status ?owner ?property ?value WHERE {
        VALUES ?rdfType { env:Environment env:Application env:Integration }
        ?entity rdf:type ?rdfType ;
                env:id ?id ;
                env:name ?name .
        BIND(STRAFTER(STR(?rdfType), STR(env:)) AS ?type)
        OPTIONAL { ?entity env:status ?status }
        OPTIONAL { ?entity env:owner ?owner }
        OPTIONAL {
          ?entity ?property ?value .
          FILTER(STRSTARTS(STR(?property), "${this.ontologyPrefix}"))
          FILTER(?property NOT IN (env:id, env:name))
        }
      }
    `;
  ...
  async getEntityById(id: string): Promise<Entity | null> {
    const escaped = this.escapeSparql(id);

    // Query 1: non-config, non-meta properties (name, type, status, owner, etc.)
    const metaQuery = `
      PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?prop ?val WHERE {
        ?entity env:id "${escaped}" .
        ?entity ?prop ?val .
        FILTER(STRSTARTS(STR(?prop), "${this.ontologyPrefix}"))
        FILTER(!STRSTARTS(STR(?prop), "${this.ontologyPrefix}config_"))
        FILTER(!STRSTARTS(STR(?prop), "${this.ontologyPrefix}meta_"))
        FILTER(?prop NOT IN (rdf:type))
      }
    `;

    // Query 2: config_ properties only (regular + bp + orphan) — may be large but scoped
    const configQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?prop ?val WHERE {
        ?entity env:id "${escaped}" .
    ...
    // Query 3: meta_ custom metadata properties
    const customMetaQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?prop ?val WHERE {
        ?entity env:id "${escaped}" .
        ?entity ?prop ?val .
    ...
    const [metaResult, configResult, customMetaResult] = await Promise.all([
      this.executeSparqlQuery(metaQuery),
      this.executeSparqlQuery(configQuery),
      this.executeSparqlQuery(customMetaQuery),
    ]);

    // Need at least the meta result to confirm entity exists
    if (!metaResult.results?.bindings?.length) {
      return null;
    }

    const entity: Record<string, any> = { id };

    for (const b of metaResult.results.bindings) {
      const propName = b.prop?.value?.replace(this.ontologyPrefix, '');
      const val = b.val?.value;
      if (propName && val !== undefined) {
        entity[propName] = val;
      }
    }

    for (const b of (configResult.results?.bindings || [])) {
      const propName = b.prop?.value?.replace(this.ontologyPrefix, '');
      const val = b.val?.value;
      if (propName && val !== undefined) {
        entity[propName] = val;
    ...
    for (const b of (customMetaResult.results?.bindings || [])) {
      const propName = b.prop?.value?.replace(this.ontologyPrefix, '');
      const val = b.val?.value;
      if (propName && val !== undefined) {
        entity[propName] = val;
  ...
  async logHistory(entityId: string, entityName: string, action: string, changes?: Record<string, any>, user?: string): Promise<void> {
    ...
    let insertData = `
      ...
      <${historyUri}> env:timestamp "${timestamp}"^^xsd:dateTime .
    `;
  ...
  /**
   * Get recent history entries across all entities within a time window.
   * Replaces the N-fan-out pattern of calling getEntityHistory per entity.
   */
  async getAllRecentHistory(options: { days?: number; limit?: number } = {}): Promise<Array<{
    id: string;
    entityId: string;
    entityName: string;
    entityType: string;
    ...
    const query = `
      ...
      SELECT ?id ?entityId ?entityName ?entityType ?action ?timestamp ?user ?changes WHERE {
        ?history env:type "History" ;
                 env:id ?id ;
                 env:entityId ?entityId ;
                 env:action ?action ;
        ...
        OPTIONAL { ?history env:user ?user }
        OPTIONAL { ?history env:changes ?changes }
        OPTIONAL { ?history env:entityName ?entityName }
        OPTIONAL {
          ?entity env:id ?entityId ;
                  env:name ?entityName .
          OPTIONAL { ?entity env:type ?entityType }
        }
        FILTER (xsd:dateTime(?timestamp) >= "${since}"^^xsd:dateTime)
      }
      ORDER BY DESC(?timestamp)
      LIMIT ${limit}
    `;
    ...
    return (result.results?.bindings || []).map((binding: any) => {
      const entry: any = {
        ...
        entityName: binding.entityName?.value || '',
        entityType: binding.entityType?.value || '',
        action: binding.action?.value || '',
        timestamp: binding.timestamp?.value || '',
      };
      if (binding.user?.value) {
        entry.user = binding.user.value;
      }
      if (binding.changes?.value) {
        try {
          entry.changes = JSON.parse(binding.changes.value);
        } catch (e) {
          entry.changes = binding.changes.value;
        }
      }
      return entry;
    });
  ...
  /**
   * Fetch only integration-change history events (create-integration and delete-integration)
   * for the last N days. Uses a targeted action FILTER so the result set is small and
   * no LIMIT truncation can hide delete events behind high-volume create events.
   */
  async getIntegrationHistory(days: number = 7): Promise<Array<{
    id: string;
    entityId: string;
    entityName: string;
    action: string;
    ...
    const query = `
      ...
      SELECT ?id ?entityId ?entityName ?action ?timestamp ?user ?changes WHERE {
        ?history env:type "History" ;
                 env:id ?id ;
                 env:entityId ?entityId ;
                 env:action ?action ;
        ...
        OPTIONAL { ?history env:changes ?changes }
        OPTIONAL { ?history env:entityName ?entityName }
        FILTER (?action IN ("create-integration", "delete-integration"))
        FILTER (xsd:dateTime(?timestamp) >= "${since}"^^xsd:dateTime)
      }
      ORDER BY DESC(?timestamp)
    `;
  ...
  async addConfigurationProperties(entityId: string, configurations: Record<string, any>): Promise<void> {
    ...
    for (const [key, value] of Object.entries(configurations)) {
      if (value !== null && value !== undefined && value !== '') {
        ...
        const maskedValue = this.maskSensitiveValue(key, String(value));
        const escapedValue = this.escapeSparql(maskedValue);
...
${deleteWherePatterns}      }
  ...
  async getNetworkOverview(): Promise<NetworkOverview> {
    const envsQuery = `
      PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?id ?name ?status ?currentState ?sourceService ?tag ?tagColor ?environmentType WHERE {
        ?entity rdf:type env:Environment ;
                env:id ?id ;
                env:name ?name .
        OPTIONAL { ?entity env:status ?status }
        ...
        OPTIONAL { ?entity env:sourceService ?sourceService }
        OPTIONAL { ?entity env:config_tag ?tag }
        OPTIONAL { ?entity env:config_tagColor ?tagColor }
        OPTIONAL { ?entity env:environmentType ?environmentType }
      }
    `;
    ...
    const nodes: EnvNode[] = (envsResult.results?.bindings || []).map((b: any) => ({
      ...
      currentState: b.currentState?.value || undefined,
      sourceService: b.sourceService?.value || undefined,
      tag: b.tag?.value || undefined,
      tagColor: b.tagColor?.value || undefined,
      environmentType: b.environmentType?.value || undefined,
    }));
  ...
  async getEnvironmentDetail(envId: string): Promise<EnvironmentDetail> {
    ...
    // Focus node: non-config, non-meta properties (small result set)
    const focusMetaQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?nodeId ?nodeName ?nodeType ?nodeStatus ?prop ?val WHERE {
    ...
    // Focus node: config_ properties only (separate query to avoid 1000-row limit)
    const focusConfigQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?nodeId ?prop ?val WHERE {
        ?entity env:id "${escapedId}" ;
                env:id ?nodeId .
        ?entity ?prop ?val .
        FILTER(STRSTARTS(STR(?prop), "${this.ontologyPrefix}config_"))
    ...
    // Focus node: meta_ custom metadata properties
    const focusCustomMetaQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?nodeId ?prop ?val WHERE {
        ?entity env:id "${escapedId}" ;
                env:id ?nodeId .
    ...
    // 2-hop: Environment neighbors via hasIntegration → integratesWith
    const neighborQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT DISTINCT ?neighborId ?neighborName ?neighborStatus WHERE {
        {
          ?relA env:type "Relationship" ;
                env:relationshipType "hasIntegration" ;
                env:sourceEntityId "${escapedId}" ;
                env:targetEntityId ?integId .
          ?relB env:type "Relationship" ;
                env:relationshipType "integratesWith" ;
                env:sourceEntityId ?integId ;
        ...
        ?neighborEntity env:id ?neighborId ;
                        env:name ?neighborName ;
                        env:type "Environment" .
        OPTIONAL { ?neighborEntity env:status ?neighborStatus }
        FILTER(?neighborId != "${escapedId}")
      }
    `;

    // Env-to-env edges involving the focused environment
    const edgesQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?envA ?envB (COUNT(DISTINCT ?integId) AS ?weight) WHERE {
        ?relA env:type "Relationship" ;
              env:relationshipType "hasIntegration" ;
              env:sourceEntityId ?envA ;
              env:targetEntityId ?integId .
        ?relB env:type "Relationship" ;
              ...
              env:targetEntityId ?envB .
        FILTER(?envA != ?envB)
        FILTER(?envA = "${escapedId}" || ?envB = "${escapedId}")
      }
      GROUP BY ?envA ?envB
    `;

    let focusMetaResult: any;
    let focusConfigResult: any;
    let focusCustomMetaResult: any;
    let neighborResult: any;
    let edgesResult: any;
    try {
      [focusMetaResult, focusConfigResult, focusCustomMetaResult, neighborResult, edgesResult] = await Promise.all([
        this.executeSparqlQuery(focusMetaQuery),
        this.executeSparqlQuery(focusConfigQuery),
        this.executeSparqlQuery(focusCustomMetaQuery),
    ...
    // Enrich neighbor nodes: fetch key properties + stats in a single batched query.
    // We use a VALUES clause so Neptune resolves all neighbors in one round-trip —
    // no N+1 overhead regardless of neighbor count.
    const neighborIds: string[] = (neighborResult.results?.bindings || [])
      .map((b: any) => b.neighborId?.value)
      .filter(Boolean);

    let neighborEnrichResult: any = { results: { bindings: [] } };
    if (neighborIds.length > 0) {
      const valuesClause = neighborIds
        .map(id => `"${this.escapeSparql(id)}"`)
        .join(' ');
      const neighborEnrichQuery = `
        PREFIX env: <${this.ontologyPrefix}>
        SELECT ?nodeId ?prop ?val WHERE {
          VALUES ?nodeId { ${valuesClause} }
          ?entity env:id ?nodeId .
          ?entity ?prop ?val .
          FILTER(STRSTARTS(STR(?prop), "${this.ontologyPrefix}"))
          FILTER(?prop NOT IN (
            <${this.ontologyPrefix}id>,
            <${this.ontologyPrefix}name>,
            <${this.ontologyPrefix}type>
          ))
          FILTER(!STRSTARTS(STR(?prop), "${this.ontologyPrefix}config_"))
          FILTER(!STRSTARTS(STR(?prop), "${this.ontologyPrefix}meta_"))
        }
      `;
      try {
        neighborEnrichResult = await this.executeSparqlQuery(neighborEnrichQuery);
      } catch (err) {
        console.warn('getEnvironmentDetail: neighbor enrichment query failed (non-fatal)', err);
      }
    }

    // Merge meta + config + custom-meta bindings
    const focusBindings: any[] = [
      ...(focusMetaResult.results?.bindings || []),
      ...(focusConfigResult.results?.bindings || []),
      ...(focusCustomMetaResult.results?.bindings || []),
    ];
    ...
    for (const b of focusBindings) {
      ...
      if (prop && val) {
        ...
        } else if (propName.startsWith('meta_')) {
          if (!focusMap[id].properties.customMetadata) {
            focusMap[id].properties.customMetadata = [];
          }
          focusMap[id].properties.customMetadata.push({ [propName.slice('meta_'.length)]: val });
        } else {
    ...
    // Build neighbor nodes: start with id/label/status from the neighbor query,
    // then overlay enriched properties and stats from the batched enrichment query.
    const neighborBindings: any[] = neighborResult.results?.bindings || [];
    const neighborMap: Record<string, NetworkNode> = {};
    for (const b of neighborBindings) {
      const id = b.neighborId?.value || '';
      if (id && !neighborMap[id]) {
        neighborMap[id] = {
          id,
    ...
    // Overlay enrichment: properties and stats (no config_ / meta_ for neighbors)
    for (const b of (neighborEnrichResult.results?.bindings || [])) {
      const id = b.nodeId?.value || '';
      const prop = b.prop?.value;
      const val = b.val?.value;
      if (!id || !prop || val === undefined) continue;
      if (!neighborMap[id]) continue;
      const propName = prop.replace(this.ontologyPrefix, '');
      if (STAT_PROPERTIES.includes(propName as any)) {
        if (!neighborMap[id].stats) neighborMap[id].stats = {};
        neighborMap[id].stats![propName as keyof EntityStats] = val;
      } else {
        neighborMap[id].properties[propName] = val;
      }
    }

    const neighbors = Object.values(neighborMap);

    // Build env-to-env edges
    const edgeBindings: any[] = edgesResult.results?.bindings || [];
    const edges: NetworkEdge[] = edgeBindings.map((b: any) => {
      const envA = b.envA?.value || '';
      const envB = b.envB?.value || '';
      const sorted = [envA, envB].sort();
      return {
  ...
  async getNodeNeighbors(nodeId: string): Promise<NeighborResult> {
    ...
    // Env-to-env edges involving this node
    const edgesQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?envA ?envB (COUNT(DISTINCT ?integId) AS ?weight) WHERE {
        ?relA env:type "Relationship" ;
  ...
  async getNodeDegrees(nodeIds: string[]): Promise<Map<string, number>> {
    ...
    // Empty array → query ALL nodes in one shot (used by the overview dashboard)
    if (nodeIds.length === 0) {
      const allQuery = `
        PREFIX env: <${this.ontologyPrefix}>
        ...
        GROUP BY ?nodeId
        ORDER BY DESC(?totalEdges)
      `;
      try {
        const allResult = await this.executeSparqlQuery(allQuery);
        for (const b of (allResult.results?.bindings || [])) {
          const id = b.nodeId?.value || '';
          const count = parseInt(b.totalEdges?.value || '0', 10);
          if (id) result.set(id, count);
        }
      } catch (error) {
        console.error('getNodeDegrees: all-nodes query error', error);
      }
      return result;
    }
  ...
  /**
   * Write a set of meta_cmdb_* triples for an entity.
   * Pass the full map of sanitised field key → display value.
   * Existing values for those keys are deleted first to avoid duplicate literals.
   * Also applies any stored field mappings to first-class EMS properties.
   */
  async writeCmdbTriples(entityId: string, fields: Record<string, string>): Promise<void> {
    if (Object.keys(fields).length === 0) return;

    // Resolve entity URI
    const escaped = this.escapeSparql(entityId);
    const uriQuery = `PREFIX env: <${this.ontologyPrefix}> SELECT ?e WHERE { ?e env:id "${escaped}" }`;
    const uriResult = await this.executeSparqlQuery(uriQuery);
    const entityUri = uriResult.results?.bindings?.[0]?.e?.value;
    if (!entityUri) throw new Error(`writeCmdbTriples: entity not found: ${entityId}`);

    // 1. Delete existing values for these specific keys (avoid duplicate literals)
    let delClauses = '';
    let delWhere = '';
    for (const k of Object.keys(fields)) {
      const varName = `v_${k.replace(/[^a-zA-Z0-9]/g, '_')}`;
      delClauses += `  <${entityUri}> env:meta_cmdb_${k} ?${varName} .\n`;
      delWhere   += `  OPTIONAL { <${entityUri}> env:meta_cmdb_${k} ?${varName} } .\n`;
    }
    const deleteQuery = `PREFIX env: <${this.ontologyPrefix}>\nDELETE {\n${delClauses}}\nWHERE  {\n${delWhere}}`;
    try { await this.executeSparqlUpdate(deleteQuery); } catch { /* fields may not exist yet */ }

    // 2. Insert new values
    const triples = Object.entries(fields)
      .map(([k, v]) => `  <${entityUri}> env:meta_cmdb_${k} "${this.escapeSparql(v)}" .`)
      .join('\n');
    const insertQuery = `PREFIX env: <${this.ontologyPrefix}>\nINSERT DATA {\n${triples}\n}`;
    await this.executeSparqlUpdate(insertQuery);

    // 3. Apply any stored field mappings (meta_cmdb_field_map_*) to first-class EMS properties
    const allProps = await this.getCmdbProperties(entityId);
    const mapping: Record<string, string> = {};
    for (const [k, v] of Object.entries(allProps)) {
      if (k.startsWith('field_map_')) {
        mapping[k.slice('field_map_'.length)] = v;
      }
    }
    if (Object.keys(mapping).length > 0) {
      await this.applyFieldMappings(entityId, mapping, fields);
    }

    console.log(`writeCmdbTriples: wrote ${Object.keys(fields).length} triples for ${entityId}`);
  }

  /**
   * Delete ALL meta_cmdb_* triples for an entity (used by unlink).
   */
  async deleteAllCmdbTriples(entityId: string): Promise<void> {
    const escaped = this.escapeSparql(entityId);
    // First look up the entity URI
    const uriQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?entity WHERE { ?entity env:id "${escaped}" }
    `;
    const uriResult = await this.executeSparqlQuery(uriQuery);
    const entityUri = uriResult.results?.bindings?.[0]?.entity?.value;
    if (!entityUri) {
      console.warn(`deleteAllCmdbTriples: no entity found for id ${entityId}`);
      return;
    }

    // Delete all meta_cmdb_* triples using DELETE { } WHERE { } with FILTER
    const deleteQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      DELETE { <${entityUri}> ?p ?o }
      WHERE  { <${entityUri}> ?p ?o .
               FILTER(STRSTARTS(STR(?p), "${this.ontologyPrefix}meta_cmdb_")) }
    `;
    await this.executeSparqlUpdate(deleteQuery);
    console.log(`deleteAllCmdbTriples: cleared all meta_cmdb_* for ${entityId}`);
  }

  /**
   * Delete specific meta_cmdb_<fieldKey> triples for an entity (used by update-fields).
   */
  async deleteCmdbFields(entityId: string, fieldKeys: string[]): Promise<void> {
    if (fieldKeys.length === 0) return;
    const patterns = fieldKeys
      .map(k => `  env:${entityId} env:meta_cmdb_${k} ?v_${k} .`)
      .join('\n');
    const query = `
...
${patterns}
    ...
    console.log(`deleteCmdbFields: removed ${fieldKeys.length} fields from ${entityId}`);
  }

  /**
   * Read all meta_cmdb_* properties for an entity.
   * Returns a plain object: strippedKey (without "meta_cmdb_" prefix) → value.
   * Keys whose name starts with "field_map_" represent mappings, not field values.
   */
  async getCmdbProperties(entityId: string): Promise<Record<string, string>> {
    const escaped = this.escapeSparql(entityId);
    const query = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?p ?o WHERE {
    ...
    const PREFIX_LEN = `${this.ontologyPrefix}meta_cmdb_`.length;
    for (const b of (result.results?.bindings || [])) {
      const fullProp: string = b.p?.value || '';
      const key = fullProp.slice(PREFIX_LEN);
      if (key) out[key] = b.o?.value || '';
    }
    return out;
  }

  /**
   * Write CMDB field → EMS property mappings as meta_cmdb_field_map_<cmdbKey> triples.
   * mapping: { cmdbFieldKey: emsPropertyName } e.g. { "assigned_to": "owner" }
   */
  async writeCmdbFieldMappings(entityId: string, mapping: Record<string, string>): Promise<void> {
    if (Object.keys(mapping).length === 0) return;
    const escaped = this.escapeSparql(entityId);
    const uriQuery = `PREFIX env: <${this.ontologyPrefix}> SELECT ?e WHERE { ?e env:id "${escaped}" }`;
    const uriResult = await this.executeSparqlQuery(uriQuery);
    const entityUri = uriResult.results?.bindings?.[0]?.e?.value;
    if (!entityUri) throw new Error(`Entity not found: ${entityId}`);

    // Delete existing mapping triples then re-insert
    const delQuery = `
      PREFIX env: <${this.ontologyPrefix}>
      DELETE { <${entityUri}> ?p ?o }
      WHERE  { <${entityUri}> ?p ?o .
               FILTER(STRSTARTS(STR(?p), "${this.ontologyPrefix}meta_cmdb_field_map_")) }
    `;
    await this.executeSparqlUpdate(delQuery);

    const triples = Object.entries(mapping)
      .map(([k, v]) => `  <${entityUri}> env:meta_cmdb_field_map_${k} "${this.escapeSparql(v)}" .`)
      .join('\n');
    const insertQuery = `PREFIX env: <${this.ontologyPrefix}>\nINSERT DATA {\n${triples}\n}`;
    await this.executeSparqlUpdate(insertQuery);
  }
  ...
  /**
   * Apply CMDB → EMS property mappings: for each mapped field whose value exists
   * in cmdbValues, upsert the corresponding first-class EMS property on the entity.
   * e.g. mapping { assigned_to: "owner" } + cmdbValues { assigned_to: "Jane" }
   *   → sets env:owner = "Jane" on the entity.
   */
  /**
   * List all EMS entities that have a linked CMDB CI (have meta_cmdb_ci_sys_id set).
   * Used by the refresh pipeline and the "All Linked CIs" UI panel.
   */
  async listLinkedCmdbEntities(): Promise<LinkedCmdbEntity[]> {
    const query = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT ?id ?name ?sysId ?ciClass ?ciName ?lastSynced ?syncStatus WHERE {
        ?e env:id        ?id ;
           env:name      ?name ;
           env:meta_cmdb_ci_sys_id  ?sysId ;
           env:meta_cmdb_ci_class   ?ciClass ;
           env:meta_cmdb_ci_name    ?ciName .
        OPTIONAL { ?e env:meta_cmdb_last_synced_at ?lastSynced }
        OPTIONAL { ?e env:meta_cmdb_sync_status    ?syncStatus }
      }
      ORDER BY ?name
    `;
    const result = await this.executeSparqlQuery(query);
    return (result.results?.bindings || []).map((b: any) => ({
      entityId:    b.id?.value      || '',
      entityName:  b.name?.value    || '',
      sysId:       b.sysId?.value   || '',
      ciClass:     b.ciClass?.value || '',
      ciName:      b.ciName?.value  || '',
      lastSynced:  b.lastSynced?.value || null,
  ...
  async applyFieldMappings(
    ...
    cmdbValues: Record<string, string>,
  ): Promise<void> {
    const toApply: Record<string, string> = {};
    for (const [cmdbKey, emsProp] of Object.entries(mapping)) {
      if (cmdbValues[cmdbKey] !== undefined) {
        toApply[emsProp] = cmdbValues[cmdbKey];
      }
    }
    ...
    let ins = '';
    let whr = '';
    for (const [prop, val] of Object.entries(toApply)) {
      const varName = `old_${prop.replace(/[^a-zA-Z0-9]/g, '_')}`;
      del += `  <${entityUri}> env:${prop} ?${varName} .\n`;
      ins += `  <${entityUri}> env:${prop} "${this.escapeSparql(val)}" .\n`;
      whr += `  OPTIONAL { <${entityUri}> env:${prop} ?${varName} } .\n`;
    }
    const upsert = `PREFIX env: <${this.ontologyPrefix}>\nDELETE {\n${del}}\nINSERT {\n${ins}}\nWHERE  {\n${whr}}`;
    await this.executeSparqlUpdate(upsert);
    console.log(`applyFieldMappings: applied ${Object.keys(toApply).length} mapped properties for ${entityId}`);
  }
}