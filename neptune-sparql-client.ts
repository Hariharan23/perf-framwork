...
export interface OrphanEnvironment extends Entity {
  ...
  endpointHint?: string;
  sourceFile?: string;
  // Orphan reason fields (set when orphaned via pipeline retarget)
  orphanReason?: string;        // e.g. 'retarget'
  orphanedAt?: string;          // ISO timestamp
  orphanedBy?: string;          // discoveredBy / pipeline name
  orphanDetail?: string;        // JSON string with extra context
  previouslyConnected?: boolean; // true when orphaned due to retarget
}
...
export class NeptuneSparqlClient {
  ...
  async getEntityById(id: string): Promise<Entity | null> {
    // Fetch all properties (including config_*, collaborators, scheduledUpdates, etc.)
    const query = `
      PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
      PREFIX env: <${this.ontologyPrefix}>
    ...
    // Ensure id is always the passed-in id (not a URI)
    entity.id = id;
   ...
   */
  /** Count how many Relationship edges involve this entity (as source or target). */
  async countEntityRelationships(entityId: string): Promise<number> {
    const query = `
      PREFIX env: <${this.ontologyPrefix}>
      SELECT (COUNT(?rel) AS ?cnt) WHERE {
        ?rel env:type "Relationship" .
        { ?rel env:sourceEntityId "${this.escapeSparql(entityId)}" }
        UNION
        { ?rel env:targetEntityId "${this.escapeSparql(entityId)}" }
      }
    `;
    const result = await this.executeSparqlQuery(query);
    const val = result.results?.bindings?.[0]?.cnt?.value;
    return val ? parseInt(val, 10) : 0;
  }

  /** Stamp 5 config_orphan* triples on an entity to record why it became an orphan. */
  async stampOrphanReason(
    entityId: string,
    reason: string,
    detail: Record<string, any>,
    triggeredBy: string,
  ): Promise<void> {
    const detailJson = this.escapeSparql(JSON.stringify(detail));
    const now = new Date().toISOString();
    const update = `
      PREFIX env: <${this.ontologyPrefix}>
      DELETE {
        ?e env:config_orphanReason ?r ;
           env:config_orphanedAt ?a ;
           env:config_orphanedBy ?b ;
           env:config_orphanDetail ?d ;
           env:config_previouslyConnected ?p .
      }
      INSERT {
        ?e env:config_orphanReason "${this.escapeSparql(reason)}" ;
           env:config_orphanedAt "${this.escapeSparql(now)}" ;
           env:config_orphanedBy "${this.escapeSparql(triggeredBy)}" ;
           env:config_orphanDetail "${detailJson}" ;
           env:config_previouslyConnected "true" .
      }
      WHERE {
        ?e env:id "${this.escapeSparql(entityId)}" .
        OPTIONAL { ?e env:config_orphanReason ?r }
        OPTIONAL { ?e env:config_orphanedAt ?a }
        OPTIONAL { ?e env:config_orphanedBy ?b }
        OPTIONAL { ?e env:config_orphanDetail ?d }
        OPTIONAL { ?e env:config_previouslyConnected ?p }
      }
    `;
    await this.executeSparqlUpdate(update);
  }

  /** Remove orphan reason triples (called when a previously-connected orphan is reconnected). */
  async clearOrphanReason(entityId: string): Promise<void> {
    const update = `
      PREFIX env: <${this.ontologyPrefix}>
      DELETE {
        ?e env:config_orphanReason ?r ;
  ...
  async listOrphanEnvironments(): Promise<OrphanEnvironment[]> {
    ...
    const query = `
      ...
      SELECT ?id ?name ?description ?discoveredBy ?discoveredAt ?rawHostname
             ...
             (SAMPLE(?orphanReason) AS ?oReason)
             (SAMPLE(?orphanedAt) AS ?oAt)
             (SAMPLE(?orphanedBy) AS ?oBy)
             (SAMPLE(?orphanDetail) AS ?oDetail)
             (SAMPLE(?prevConnected) AS ?oPrevConn)
      WHERE {
        ...
        OPTIONAL { ?entity env:config_orphanReason ?orphanReason }
        OPTIONAL { ?entity env:config_orphanedAt ?orphanedAt }
        OPTIONAL { ?entity env:config_orphanedBy ?orphanedBy }
        OPTIONAL { ?entity env:config_orphanDetail ?orphanDetail }
        OPTIONAL { ?entity env:config_previouslyConnected ?prevConnected }
        OPTIONAL {
    ...
    return (result.results?.bindings || []).map((b: any) => ({
      ...
      orphanReason: b.oReason?.value || '',
      orphanedAt: b.oAt?.value || '',
      orphanedBy: b.oBy?.value || '',
      orphanDetail: b.oDetail?.value || '',
      previouslyConnected: b.oPrevConn?.value === 'true',
    }));
  }
  ...
  async connectOrphanNode(
    ...
    // 0. Clear any orphan-reason stamps (node is being reconnected)
    await this.clearOrphanReason(orphanId);
...
${deleteWherePatterns}      }
  ...
  async getEnvironmentDetail(envId: string): Promise<EnvironmentDetail> {
    ...
    // Focus node: fetch full properties
    const focusQuery = `
      PREFIX env: <${this.ontologyPrefix}>
    ...
    let edgesResult: any;
    try {
      [focusResult, neighborResult, edgesResult] = await Promise.all([
        this.executeSparqlQuery(focusQuery),
    ...
    // Build focus node
    const focusBindings: any[] = focusResult.results?.bindings || [];
