...
${deleteWherePatterns}      }
  ...
  async writeCmdbTriples(entityId: string, fields: Record<string, string>): Promise<void> {
    ...
    const prefix = 'PREFIX env: <http://neptune.aws.com/envmgmt/ontology/>';

    // 1. Delete existing values for these specific keys (avoid duplicate literals)
    const delPatterns = Object.keys(fields)
      .map(k => `  env:${entityId} env:meta_cmdb_${k} ?v_${k} .`)
      .join('\n');
    const deleteQuery = `${prefix}\nDELETE WHERE {\n${delPatterns}\n}`;
    try {
      await this.executeSparqlUpdate(deleteQuery);
    } catch {
      // Ignore — fields may not exist yet
    }

    // 2. Insert new values
    const triples = Object.entries(fields)
      .map(([k, v]) => `  env:${entityId} env:meta_cmdb_${k} "${escapeSparql(v)}" .`)
      .join('\n');
    const insertQuery = `${prefix}\nINSERT DATA {\n${triples}\n}`;
    await this.executeSparqlUpdate(insertQuery);
  ...
  async deleteAllCmdbTriples(entityId: string): Promise<void> {
    const query = `
      ...
      DELETE WHERE {
        env:${entityId} ?p ?o .
        FILTER(STRSTARTS(STR(?p), STR(env:meta_cmdb_)))
      }
    `;
...
${patterns}
  ...
  async getCmdbProperties(entityId: string): Promise<Record<string, string>> {
    ...
    const PREFIX_LEN = 'http://neptune.aws.com/envmgmt/ontology/meta_cmdb_'.length;
