...
export class NeptuneSparqlClient {
  ...
  async getEntityById(id: string): Promise<Entity | null> {
    ...
    // Query 1: non-config properties (name, type, status, owner, etc.) — small result set
    const metaQuery = `
      PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
      PREFIX env: <${this.ontologyPrefix}>
...
${deleteWherePatterns}      }
  ...
  async getEnvironmentDetail(envId: string): Promise<EnvironmentDetail> {
    ...
    // Focus node: non-config properties (small result set)
    ...
    let edgesResult: any;
    try {
      [focusMetaResult, focusConfigResult, neighborResult, edgesResult] = await Promise.all([
    ...
    // Merge meta + config bindings; config rows lack nodeName/nodeType so carry them over
