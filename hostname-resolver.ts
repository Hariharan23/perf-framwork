...
export interface HostnameResolverConfig {
  ...
  /**
   * When set, Neptune lookup (Tier 3) is restricted to environments whose
   * `config_discoveredBy` triple matches this scope string.
   * Format: "<SOURCE_TYPE>:<instanceId>" e.g. "PAS:pasDatasource"
   *
   * This prevents hostname matches from crossing source boundaries when
   * a `resolve-scope` topology edge is configured for the current pipeline instance.
   */
  discoveredByScope?: string;
}
...
export class HostnameResolver {
   ...
   * When `discoveredByScope` is configured, only environments whose
   * `config_discoveredBy` value matches the scope are considered.
   */
  private async searchNeptuneForHostname(hostname: string): Promise<string | null> {
    const scope = this.config.discoveredByScope;
    try {
      // Build an optional scope filter: if discoveredByScope is set we require
      // env:config_discoveredBy to equal the scope string; otherwise any env matches.
      const scopeFilter = scope
        ? `OPTIONAL { ?entity env:config_discoveredBy ?srcScope }
           FILTER(BOUND(?srcScope) && ?srcScope = "${scope}")`
        : '';
