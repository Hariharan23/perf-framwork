  ...
  constructor(scope: Construct, id: string, props: AliasMgrStackProps) {
    ...
    const aliasMgrApi = new apigateway.RestApi(this, 'SRE-POC-AliasMgrApi', {
      ...
      defaultCorsPreflightOptions: {
        ...
        allowHeaders: ['Content-Type', 'x-api-key', 'X-Api-Key', 'Authorization'],
    ...
    const aliasMgrResource = aliasMgrApi.root.addResource('alias-manager');
    aliasMgrResource.addMethod('POST', new apigateway.LambdaIntegration(aliasMgrLambda), {
      apiKeyRequired: true,
    });
    aliasMgrResource.addMethod('GET', new apigateway.LambdaIntegration(aliasMgrLambda), {
      apiKeyRequired: true,
    });

    // ── Outputs ───────────────────────────────────────────────────────────
    // Gateway Responses — CORS headers on API Gateway-generated 4xx/5xx (e.g. 403 missing key, 502)
    aliasMgrApi.addGatewayResponse('AliasMgr4xxCors', {
    ...
    aliasMgrApi.addGatewayResponse('AliasMgr5xxCors', {
