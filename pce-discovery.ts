...
async function handleJsonOperation(payload: any): Promise<any> {
  ...
  switch (operation) {
    ...
    case 'connect-orphan-node': {
      const { nodeId, orphanName, sourceEnvName } = payload;
      ...
      // Create a direct integratesWith relationship: sourceEnv → orphan node
      await neptuneClient.createRelationship(sourceEnvName, 'integratesWith', orphanName, undefined, nodeId);
      return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ success: true }) };
    }
