...
function orchestratorConfig(): OrchestratorConfig {
  return {
...
export async function handler(event: SyncEvent): Promise<any> {
  ...
  try {
    ...
    await runDiscoveryPipeline(fetchResult.records, orchestratorConfig(), 'pce-database');
    ...
    // 6. Record success
    await writeRun(pipelineId, runId, 'success', fetchResult.records.length, durationMs, '', triggeredBy);
