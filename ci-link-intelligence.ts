...
import { canonicalizeIdentity, ciClassPreference, compareCanonicalIdentities } from '../shared/cmdb/ci-match-engine';
...
async function suggest(entity: EntityIdentity, adapter: ServiceNowAdapter, aliases: AliasRecord[]): Promise<any[]> {
  ...
  const emsIdentity = canonicalizeIdentity(entity.name);
  const signals = [...new Set([
    entity.hostname, entity.uniqueIdentifier, entity.name, ...entityAliases,
    ...emsIdentity.searchVariants,
  ...
  const shortlist = [...found.values()]
    .sort((a, b) => compareCanonicalIdentities(emsIdentity, canonicalizeIdentity(b.name)).score - compareCanonicalIdentities(emsIdentity, canonicalizeIdentity(a.name)).score)
    .slice(0, 8);
  ...
  for (const { candidate: c, detail } of detailed) {
    if (!detail) continue;
    const canonical = compareCanonicalIdentities(emsIdentity, canonicalizeIdentity(c.name));
    let score = canonical.score;
    const evidence: string[] = [...canonical.evidence];
    const aliasMatch = entityAliases.some(a => norm(a) === norm(c.name) || norm(a) === norm(detail.fields.fqdn || detail.fields.host_name));
    const hostnameMatch = !!entity.hostname && [detail.fields.fqdn, detail.fields.host_name, detail.fields.name].some(v => norm(v) === norm(entity.hostname));
    const ipMatch = !!entity.ipAddress && detail.fields.ip_address === entity.ipAddress;
    const externalIdentityMatch = aliasMatch || hostnameMatch || ipMatch;
    const identityEligible = canonical.identityEligible || externalIdentityMatch;
    const classPreference = ciClassPreference(entity.type, c.ciClass);
    if (identityEligible) { score += classPreference.score; evidence.push(`${classPreference.label}: ${c.ciClass}`); }
    if (aliasMatch) { score += 45; evidence.push('exact EMS alias match'); }
    if (hostnameMatch) { score += 50; evidence.push('exact hostname/FQDN match'); }
    if (ipMatch) { score += 40; evidence.push('exact IP address match'); }
    if (!identityEligible) score = 0;
    const state = lifecycleState(detail.fields);
    if (state !== 'OPERATIONAL') { score -= 60; evidence.push(`CI is ${state.toLowerCase()}`); }
    ranked.push({ sysId: c.sysId, ciClass: c.ciClass, ciName: c.name, confidence: Math.max(0, Math.min(100, score)), lifecycleState: state,
      evidence, numericConflict: canonical.numericConflict, environmentConflict: canonical.environmentConflict,
      identityEligible, identityStrength: externalIdentityMatch ? 'DETERMINISTIC' : canonical.identityStrength,
      canonicalEmsName: emsIdentity.canonicalBase, canonicalCiName: canonicalizeIdentity(c.name).canonicalBase,
      classCompatibility: classPreference.label });
  }
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    if (operation === 'suggestions') {
      ...
      for (let i = 0; i < page.length; i += 5) {
        ...
        const results = await Promise.all(batch.map(async entity => {
          ...
          const crossClassAmbiguity = !!top && !!second && top.ciClass !== second.ciClass && top.confidence - second.confidence < 10;
          const safeForAuto = top?.confidence >= 95 && top.identityEligible && top.identityStrength === 'DETERMINISTIC'
            && top.lifecycleState === 'OPERATIONAL' && !top.numericConflict
            && !top.environmentConflict && top.classCompatibility === 'preferred CI class' && !crossClassAmbiguity;
          const recommendationState = crossClassAmbiguity ? 'AMBIGUOUS' : safeForAuto ? 'AUTO_LINK_ELIGIBLE' : top?.confidence >= 70 ? 'REVIEW' : 'UNMATCHED';
          return { entity, recommendationState, ambiguityReason: crossClassAmbiguity ? 'Top candidates from different CI classes are within 10 confidence points.' : null, candidates };
        }));
    ...
    if (operation === 'health' || operation === 'links') {
      ...
      const neptuneLinkedIds = new Set(legacyLinks.map(link => link.entityId));

      // Reconcile in the opposite direction too. If the Environment page has
      // removed meta_cmdb_* data, Neptune no longer reports that entity and the
      // intelligence record must stop presenting it as linked.
      const removedExternally = links.filter(link => link.linkStatus === 'LINKED' && !neptuneLinkedIds.has(link.emsEntityId));
      for (let i = 0; i < removedExternally.length; i += 20) {
        await Promise.all(removedExternally.slice(i, i + 20).map(async link => {
          await ddb.send(new UpdateItemCommand({
            TableName: LINKS, Key: marshall({ emsEntityId: link.emsEntityId }),
            UpdateExpression: 'SET linkStatus=:s, healthStatus=:h, unlinkedAt=:u, unlinkedBy=:b, updatedAt=:u',
            ExpressionAttributeValues: marshall({ ':s': 'UNLINKED', ':h': 'NOT_APPLICABLE', ':u': importedAt, ':b': 'environment-page' }),
          }));
          await ddb.send(new PutItemCommand({ TableName: EVENTS, Item: marshall({
            emsEntityId: link.emsEntityId, eventId: `${importedAt}#${randomUUID()}`,
            eventType: 'LINK_REMOVED_FROM_ENVIRONMENT_PAGE', createdAt: importedAt,
            actor: 'environment-page', details: JSON.stringify(link),
          }) }));
          link.linkStatus = 'UNLINKED'; link.healthStatus = 'NOT_APPLICABLE';
          link.unlinkedAt = importedAt; link.unlinkedBy = 'environment-page'; link.updatedAt = importedAt;
        }));
      }
